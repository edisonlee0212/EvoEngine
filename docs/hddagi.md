...
Status: H0-H5 implementation and focused validation are complete. HDDAGI provides deferred diffuse/specular lighting and diffuse illumination for reflection captures. H6 comparative visual and performance validation remains pending. HDDAGI remains opt-in; SDFGI is still the default.
...
Camera diffuse/specular targets use R32UI storage with RGB9E5 sampled views and a separate RG8 coverage image. GI and sharp-reflection dimensions both use the viewport or floor(viewport/2), clamped to one for tiny cameras. The reference traces one ray per eligible GI pixel with 2x2 direction variation and coplanar blending; it does not trace at one quarter of the GI dimensions. The gather roughness transition is 0.25; the optional bilateral reflection filter has a separate 0.3 cutoff and radius 6/12 for half/full resolution. Two filter passes use independent scratch images. The final deferred pass upsamples and composes incoming scene-linear radiance with native material response, AO and local-reflection/SSR priority.
