...
| DDGI replacing only diffuse IBL | Retain | `mix(diffuse_ibl, ddgi_diffuse, coverage * confidence)` preserves valid black probes and uncovered diffuse fallback. Split-sum specular still samples reflection probes/global fallback; DDGI visibility may occlude rough probe specular, but DDGI irradiance never replaces specular radiance. Numerical and shader tests cover exact-metal behavior. |
...
| Runtime policy, variability, debug-readback, layout, and validation-report mirrors | Narrow | The unused lighting-policy bit and write-only variability-valid bit are gone. Debug counts/readiness derive from downloaded values, and the obsolete index-memory report value remains a schema-preserving literal zero. The later localized-convergence work deliberately expanded variability readback to four floats for sum, maximum, unstable count, and weight. |
