...
//=========================================================================================
layout(std140, binding = 0) readonly buffer SCENE_LEVEL_BVH_BLOCK {
  vec4 scene_level_bvh_nodes[];
};

layout(std140, binding = 1) readonly buffer NODE_INDICES_BLOCK {
  vec4 node_indices[];
};

layout(std140, binding = 2) readonly buffer NODE_INFO_LIST_BLOCK {
  vec4 node_info_list[];
};

layout(std140, binding = 3) readonly buffer NODE_LEVEL_BVH_NODES_BLOCK {
  vec4 node_level_bvh_nodes[];
};

layout(std140, binding = 4) readonly buffer MESH_INDICES_BLOCK {
  vec4 mesh_indices[];
};

layout(std140, binding = 5) readonly buffer MESH_INFO_LIST_BLOCK {
  vec4 mesh_info_list[];
};

layout(std140, binding = 6) readonly buffer MESH_LEVEL_BVH_NODES_BLOCK {
  vec4 mesh_level_bvh_nodes[];
};

layout(std140, binding = 7) readonly buffer TRIANGLE_INDICES_BLOCK {
  vec4 triangle_indices[];
};

layout(std140, binding = 8) readonly buffer LOCAL_TRIANGLE_INDICES_BLOCK {
  vec4 local_triangle_indices[];
};

layout(std140, binding = 9) readonly buffer SCENE_TRIANGLES_BLOCK {
  vec4 scene_triangles[];
};

layout(std140, binding = 10) readonly buffer SCENE_VERTEX_POSITIONS_BLOCK {
  vec4 scene_vertex_positions[];
};

layout(std140, binding = 11) readonly buffer SCENE_INFO_BLOCK {
  vec4 scene_info;
};
...
//| Main API                                                                              |
//=========================================================================================
HitInfo Trace(in RayDescriptor ray_descriptor, in float cull_back_face, in float cull_front_face, out bool has_hit);
        ...
        for (uint test_mesh_index = mesh_group.begin_next_level_element_index + mesh_indices_offset;
          ...
          while (triangle_group_index < mesh_level_bvh_node_size) {
            ...
            for (uint test_triangle_index = triangle_group.begin_next_level_element_index + triangle_indices_offset;
              ...
              if (cull_back_face != 0.0 && normal_test > 0.0)
                continue;
              if (cull_front_face != 0.0 && normal_test < 0.0)
                continue;
              if (normal_test == 0.0)
                continue;
