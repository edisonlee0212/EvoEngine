...
bool OptiXRayTracer::RenderToCamera(const EnvironmentProperties &environmentProperties, CameraProperties &cameraProperties,
...
void OptiXRayTracer::EstimateIllumination(const size_t &size, const EnvironmentProperties &environmentProperties,
...
void OptiXRayTracer::ScanPointCloud(const size_t &size, const EnvironmentProperties &environmentProperties,
                               CudaBuffer &samples) {
  if (!m_hasAccelerationStructure)
    return;
  if (size == 0) {
