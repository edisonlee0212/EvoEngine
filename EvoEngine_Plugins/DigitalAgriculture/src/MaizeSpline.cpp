...
MaizeSplineSegment::MaizeSplineSegment(const glm::vec3& position, const glm::vec3& up, const glm::vec3& front,
...
glm::vec3 MaizeSplineSegment::GetLeafPoint(const float angle) const {
...
glm::vec3 MaizeSplineSegment::GetStemPoint(const float angle) const {
  const auto direction = glm::rotate(up, glm::radians(angle), front);
  return position - radius * direction;
}

glm::vec3 MaizeSplineSegment::GetNormal(const float angle) const {
  return glm::normalize(glm::rotate(up, glm::radians(angle), front));
}

void MaizeSpline::SubdivideByDistance(const float subdivision_distance,
                                        std::vector<MaizeSplineSegment>& subdivided_segments) const {
...
void MaizeSpline::Serialize(const std::string& name, YAML::Emitter& out) const {
  out << YAML::Key << name << YAML::BeginMap;
  Serialization::SerializeVector("segments", segments, out);
  out << YAML::EndMap;
}

void MaizeSpline::Deserialize(const std::string& name, const YAML::Node& in) {
  if (in[name])
    Serialization::DeserializeVector("segments", segments, in[name]);
}

void MaizeSpline::GetPositionControlPoints(const uint32_t segment_index, glm::vec3& p0, glm::vec3& p1, glm::vec3& p2,
                                             glm::vec3& p3) const {
...
float MaizeSpline::GetSegmentArcLength(const uint32_t segment_index, const float t_start, const float t_end,
...
float MaizeSpline::GetArcLength(const float tolerance) const {
  float sum = 0.f;
  for (int i = 0; i < segments.size() - 1; i++) {
...
MaizeSplineSegment MaizeSpline::InterpolateSegment(const uint32_t segment_index, const float t) const {
  MaizeSplineSegment ret_val;
...
std::vector<MaizeSplineSegment> MaizeSpline::RebuildFixedSizeSegments(const size_t segment_count,
                                                                          const float tolerance) const {
  const float total_arc_length = GetArcLength(tolerance);
  const float target_segment_length = total_arc_length / segment_count;
  std::vector<MaizeSplineSegment> new_segments;
  float t = 0.f;
...
std::vector<MaizeSplineSegment> MaizeSpline::RebuildFixedLengthSegments(const float segment_length,
                                                                            const float tolerance) const {
  std::vector<MaizeSplineSegment> new_segments;
  float t = 0.f;
...
std::vector<MaizeSplineSegment> MaizeSpline::GetStemPart() const {
  std::vector<MaizeSplineSegment> ret_val;
...
std::vector<MaizeSplineSegment> MaizeSpline::GetLeafPart() const {
  std::vector<MaizeSplineSegment> ret_val;
