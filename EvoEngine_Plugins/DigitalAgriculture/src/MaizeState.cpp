...
bool MaizePanicleState::OnInspectImpl() {
  bool changed = false;
...
void MaizePanicleState::Serialize(YAML::Emitter& out) const {
...
void MaizePanicleState::Deserialize(const YAML::Node& in) {
  if (in["panicle_size"])
...
void MaizePanicleState::Apply(MaizePanicleDescriptor& target_maize_panicle_descriptor) const {
  target_maize_panicle_descriptor.panicle_size = panicle_size;
  target_maize_panicle_descriptor.seed_amount = seed_amount;
  target_maize_panicle_descriptor.seed_radius = seed_radius;
}

MaizePanicleState::MaizePanicleState() {
...
glm::vec3 MaizeStemState::GetPoint(const float point) const {
  return direction * point * length;
}

void MaizeStemState::Serialize(YAML::Emitter& out) const {
...
void MaizeStemState::Deserialize(const YAML::Node& in) {
  if (in["spline"]) {
...
bool MaizeStemState::OnInspectImpl(int mode) {
  bool changed = false;
  switch (static_cast<MaizeStateMode>(mode)) {
    case MaizeStateMode::Default:
      // ImGui::DragFloat3("Direction", &direction.x, 0.01f);
      if (ImGui::DragFloat("Length", &length, 0.01f))
        changed = true;
      break;
    case MaizeStateMode::CubicBezier:
...
void MaizeStemState::Apply(MaizeStemDescriptor& target_maize_stem_descriptor) const {
  target_maize_stem_descriptor.spline.segments.clear();
  const auto maize_layer = Application::GetLayer<MaizeLayer>();
  const int stem_node_amount = static_cast<int>(glm::max(4.0f, length / maize_layer->vertical_subdivision_length));
  ...
  for (int i = 0; i <= stem_node_amount; i++) {
    ...
    target_maize_stem_descriptor.spline.segments.emplace_back(stem_node_position, up, direction, stem_width, 180.f, 0,
                                                                0);
  }
}

bool MaizeLeafState::OnInspectImpl(int mode) {
  bool changed = false;
...
void MaizeLeafState::Apply(const MaizeStemState& stem_state,
                             MaizeLeafDescriptor& target_maize_leaf_descriptor) const {
  target_maize_leaf_descriptor.spline.segments.clear();
  target_maize_leaf_descriptor.index = index;
  target_maize_leaf_descriptor.current_growth = current_growth;
  // If the leaf is marked as dead or has effectively zero length, we skip generating geometry segments.
  // This prevents generating degenerate geometry (0-length splines) which can cause flickering or other rendering issues.
  if (dead || length <= 0.0001f) {
    return;
  }
  ...
  if (sheath_ratio > 0) {
    int root_to_sheath_node_count =
        static_cast<int>(glm::min(2.0f, stem_state.length * sheath_ratio / maize_layer->vertical_subdivision_length));
    for (int i = 0; i < root_to_sheath_node_count; i++) {
      ...
      target_maize_leaf_descriptor.spline.segments.emplace_back(
  ...
  int node_amount = static_cast<int>(glm::max(4.0f, length / maize_layer->vertical_subdivision_length));
  float unit_length = length / static_cast<float>(node_amount);

  int node_to_full_expand = static_cast<int>(0.1f * length / maize_layer->vertical_subdivision_length);
  if (node_to_full_expand == 0) node_to_full_expand = 1;
...
void MaizeLeafState::Serialize(YAML::Emitter& out) const {
...
void MaizeLeafState::Deserialize(const YAML::Node& in) {
  if (in["index"])
...
void MaizeLeafState::CopyShape(const MaizeLeafState& another) {
...
bool MaizeState::OnInspectImpl(int mode) {
  bool changed = false;
  ...
  if (mode == static_cast<int>(MaizeStateMode::CubicBezier)) {
    FileUtils::OpenFile(
        "Import...", "TXT", {".txt"},
...
bool MaizeState::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  bool changed = false;
  if (ImGui::Button("Instantiate")) {
    const auto new_entity = CreateEntity("New Maize");
  }

  static float target_waviness_factor = 1.0f;
  ImGui::DragFloat("Target leaf waviness", &target_waviness_factor, 0.01f, 0.01f, 3.0f);
  if (ImGui::Button("Create maize with changed waviness")) {
    const auto scene = Application::GetActiveScene();
    const auto maize_entity = scene->CreateEntity(GetTitle());
    const auto maize = scene->GetOrSetPrivateComponent<Maize>(maize_entity).lock();
    const auto new_maize_state = AssetManager::CreateTemporaryAsset<MaizeState>();
    MaizeMeshGeneratorSettings settings{};
    settings.enable_leaf_sheath = false;
    settings.bottom_face = false;
    ChangeWaviness(target_waviness_factor, settings, *new_maize_state);

    maize->maize_state = new_maize_state;
    if (const auto maize_layer = Application::GetLayer<MaizeLayer>()) {
      maize->GenerateGeometryEntities(maize_layer->maize_mesh_generator_settings);
    } else {
      maize->GenerateGeometryEntities({});
    }
  }

  static int state_mode = static_cast<int>(MaizeStateMode::Default);
...
void MaizeState::Apply(const std::shared_ptr<MaizeDescriptor>& target_maize_descriptor) const {
  panicle.Apply(target_maize_descriptor->panicle);
  stem.Apply(target_maize_descriptor->stem);
  target_maize_descriptor->leaves.resize(leaves.size());
  for (int leaf_index = 0; leaf_index < leaves.size(); leaf_index++) {
    leaves[leaf_index].Apply(stem, target_maize_descriptor->leaves[leaf_index]);
  }
}

void MaizeState::Serialize(YAML::Emitter& out) const {
...
void MaizeState::Deserialize(const YAML::Node& in) {
  if (in["version_"])
...
Entity MaizeState::CreateEntity(const std::string& name) const {
  const auto scene = Application::GetActiveScene();
  const auto maize_entity = scene->CreateEntity(name);
  const auto maize = scene->GetOrSetPrivateComponent<Maize>(maize_entity).lock();
  maize->maize_state = GetSelf();
  if (const auto maize_layer = Application::GetLayer<MaizeLayer>()) {
    maize->GenerateGeometryEntities(maize_layer->maize_mesh_generator_settings);
  } else {
    maize->GenerateGeometryEntities({});
...
std::shared_ptr<Texture2D> MaizeState::GenerateThumbnailTexture() {
...
void MaizeState::ChangeWaviness(const float factor, const MaizeMeshGeneratorSettings& mesh_generator_settings,
                                  MaizeState& target_maize_state) const {
  target_maize_state = *this;
  for (int leaf_index = 0; leaf_index < target_maize_state.leaves.size(); leaf_index++) {
    leaves[leaf_index].ChangeWaviness(factor, stem, mesh_generator_settings, target_maize_state.leaves[leaf_index]);
  }
}

void MaizeLeafState::ChangeWaviness(const float factor, const MaizeStemState& stem_state,
  ...
  MaizeLeafDescriptor maize_leaf_descriptor;

  Apply(stem_state, maize_leaf_descriptor);
  maize_leaf_descriptor.GenerateGeometry(vertices, indices, mesh_generator_settings);
  ...
  for (int iteration = 0; iteration < 16; iteration++) {
    ...
    target_leaf_state.Apply(stem_state, maize_leaf_descriptor);
