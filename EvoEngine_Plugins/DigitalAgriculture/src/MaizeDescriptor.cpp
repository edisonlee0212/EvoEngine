...
#include "IVolume.hpp"
#include "Maize.hpp"
// #include "MaizeDescriptorReconstruction.hpp" // Not available yet
#include "MaizeLayer.hpp"
#include "MaizeSpline.hpp"
#include "assimp/code/AssetLib/3MF/3MFXmlTags.h"
using namespace digital_agriculture_plugin;

bool MaizeMeshGeneratorSettings::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  if (ImGui::TreeNode("Maize mesh generator settings")) {
...
bool MaizePanicleDescriptor::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  return false;
}

void MaizePanicleDescriptor::Serialize(YAML::Emitter& out) const {
...
void MaizePanicleDescriptor::Deserialize(const YAML::Node& in) {
  if (in["panicle_size"])
...
void MaizePanicleDescriptor::GenerateGeometry(const glm::vec3& stem_tip, std::vector<Vertex>& vertices,
...
bool MaizeStemDescriptor::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  bool changed = false;
...
void MaizeStemDescriptor::Serialize(YAML::Emitter& out) const {
  spline.Serialize("spline", out);
}

void MaizeStemDescriptor::Deserialize(const YAML::Node& in) {
  spline.Deserialize("spline", in);
}

void MaizeStemDescriptor::GenerateGeometry(std::vector<Vertex>& vertices, std::vector<unsigned>& indices) const {
  if (spline.segments.empty())
    return;
  auto maize_layer = Application::GetLayer<MaizeLayer>();
  if (!maize_layer)
    return;
  std::vector<MaizeSplineSegment> segments;
  spline.SubdivideByDistance(maize_layer->vertical_subdivision_length, segments);
  ...
  const float x_step = 1.0f / maize_layer->horizontal_subdivision_step / 2.0f;
  ...
  for (int i = 0; i < segment_size; i++) {
    ...
    const float angle_step = segment.theta / maize_layer->horizontal_subdivision_step;
    const int verts_count = maize_layer->horizontal_subdivision_step * 2 + 1;
    for (int j = 0; j < verts_count; j++) {
      const auto position = segment.GetStemPoint((j - maize_layer->horizontal_subdivision_step) * angle_step);
...
bool MaizeLeafDescriptor::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  bool changed = false;
  for (int i = 0; i < spline.segments.size(); i++) {
...
void MaizeLeafDescriptor::Serialize(YAML::Emitter& out) const {
  out << YAML::Key << "index" << YAML::Value << index;
  spline.Serialize("spline", out);
}

void MaizeLeafDescriptor::Deserialize(const YAML::Node& in) {
  if (in["index"])
    index = in["index"].as<int>();
  spline.Deserialize("spline", in);
}

void MaizeLeafDescriptor::GenerateGeometry(std::vector<Vertex>& vertices, std::vector<unsigned>& indices,
  ...
  if (!maize_layer)
    return;
  std::vector<MaizeSplineSegment> segments;  // = spline.segments;
  MaizeSpline temp_spline;
  spline.SubdivideByDistance(maize_layer->vertical_subdivision_length, temp_spline.segments);
...
#pragma endregion
  ...
  const float x_step = 1.0f / static_cast<float>(maize_layer->horizontal_subdivision_step) / 2.0f;
  auto segment_size = segments.size();
  const float y_leaf_step = 0.5f / segment_size;

  for (int i = 0; i < segment_size; i++) {
    auto& segment = segments.at(i);
    const float angle_step = segment.theta / static_cast<float>(maize_layer->horizontal_subdivision_step);
    const int verts_count = maize_layer->horizontal_subdivision_step * 2 + 1;
    for (int j = 0; j < verts_count; j++) {
      auto position =
          segment.GetLeafPoint((j - static_cast<float>(maize_layer->horizontal_subdivision_step)) * angle_step);
      auto normal =
          segment.GetNormal((j - static_cast<float>(maize_layer->horizontal_subdivision_step)) * angle_step);
  ...
  const glm::vec3 start_position = segments.at(0).position;
  // Apply scaling to the leaf geometry based on current_growth
  if (current_growth < 1.0f) {
    const float scale_factor = glm::mix(0.1f, 1.0f, current_growth);
    for(int i = vertex_index; i < vertices.size(); i++){
        vertices[i].position = start_position + (vertices[i].position - start_position) * scale_factor;
    }
  }
}

bool MaizeDescriptor::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  if (ImGui::Button("Instantiate")) {
    CreateEntity("New Maize");
  }
...
void MaizeDescriptor::Serialize(YAML::Emitter& out) const {
...
void MaizeDescriptor::Deserialize(const YAML::Node& in) {
  if (in["panicle"])
  ...
  if (in["leaves"]) {
    for (const auto& i : in["leaves"]) {
      MaizeLeafDescriptor leaf_state{};
...
Entity MaizeDescriptor::CreateEntity(const std::string& name) const {
  ...
  maize->maize_descriptor = GetSelf();

  if (const auto maize_layer = Application::GetLayer<MaizeLayer>()) {
    maize_layer->maize_mesh_generator_settings.enable_stem = true;
...
std::shared_ptr<Texture2D> MaizeDescriptor::GenerateThumbnailTexture() {
  ...
  if (!thumbnail) {
    ...
    thumbnail->Import(std::filesystem::absolute(std::filesystem::path("./DigitalAgricultureResources") /
                                                "Icons/SorghumDescriptor.png")); // Re-using for now
  }
  return thumbnail;
}

std::optional<std::vector<std::unordered_map<std::string, std::vector<glm::vec3>>>> MaizeDescriptor::ImportPrediction(
    const std::filesystem::path& yaml_path) {
  // Logic from SorghumDescriptor::ImportPrediction
  // Re-used for Maize if format is same
  ...
  try {
    ...
    // Looking for "Sorghum" key? Maybe should look for "Maize"?
    // The prompt says "Any data in our new Maize counterpart will just use the Sorghum data for now."
    // So likely the logic is same.
    if (in["Sorghum"]) { 
      const auto& maize_in = in["Sorghum"];
      for (const auto maize_real_in : maize_in) {
        if (maize_real_in["Leaves"]) {
          const auto& leaves_in = maize_real_in["Leaves"];
