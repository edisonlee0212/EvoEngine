...
void MaizeGrowthStages::Apply(const std::shared_ptr<MaizeDescriptor>& target_maize_descriptor,
                                const float time) const {
  if (maize_growth_stages.empty())
    return;
  const auto actual_time = glm::clamp(time, 0.0f, 99999.0f);
  float previous_time = maize_growth_stages.begin()->first;
  MaizeGrowthStagePair state_pair;
  state_pair.left_stage = maize_growth_stages.begin()->second;
  state_pair.right_stage = state_pair.left_stage;

  if (actual_time < previous_time) {
    // Get from zero state to first state.
    state_pair.Apply(target_maize_descriptor, 0.0f);
    return;
  }

  float a = 0.0f;
  for (auto it = (++maize_growth_stages.begin()); it != maize_growth_stages.end(); ++it) {
  ...
  state_pair.Apply(target_maize_descriptor, a);
}

void MaizeGrowthStagePair::Apply(const std::shared_ptr<MaizeDescriptor>& target_maize_descriptor,
                                   const float a) const {
  ApplyPanicle(target_maize_descriptor, a);
  ApplyStem(target_maize_descriptor, a);
  ApplyLeaves(target_maize_descriptor, a);
}

void MaizeGrowthStagePair::ApplyLeaves(const std::shared_ptr<MaizeDescriptor>& target_descriptor,
                                         const float a) const {
...
void MaizeGrowthStagePair::ApplyLeaf(const std::shared_ptr<MaizeDescriptor>& target_descriptor, float a,
  ...
  MaizeLeafState actual_left, actual_right;
  ...
  if (sheath_ratio > 0) {
    int root_to_sheath_node_count =
        glm::min(2.0f, stem_length * sheath_ratio / maize_layer->vertical_subdivision_length);
  ...
  int sheath_node_count = glm::max(2.0f, stem_length * back_track_ratio / maize_layer->vertical_subdivision_length);
  ...
  int node_amount = glm::max(4.0f, leaf_length / maize_layer->vertical_subdivision_length);
  float unit_length = leaf_length / node_amount;

  int node_to_full_expand = 0.1f * leaf_length / maize_layer->vertical_subdivision_length;
...
void MaizeGrowthStagePair::LeafStateHelper(MaizeLeafState& left, MaizeLeafState& right, float& a,
...
int MaizeGrowthStagePair::GetLeafSize(const float a) const {
...
float MaizeGrowthStagePair::GetStemLength(const float a) const {
  float left_length, right_length;
  switch (static_cast<MaizeStateMode>(state_mode)) {
...
glm::vec3 MaizeGrowthStagePair::GetStemDirection(const float a) const {
...
glm::vec3 MaizeGrowthStagePair::GetStemPoint(const float a, const float point) const {
...
void MaizeGrowthStagePair::ApplyPanicle(const std::shared_ptr<MaizeDescriptor>& target_state, const float a) const {
...
void MaizeGrowthStagePair::ApplyStem(const std::shared_ptr<MaizeDescriptor>& target_state, const float a) const {
  constexpr auto up_direction = glm::vec3(0, 1, 0);
  auto front_direction = glm::vec3(0, 0, -1);
  front_direction = glm::rotate(front_direction, glm::radians(glm::linearRand(0.0f, 360.0f)), up_direction);
  glm::vec3 stem_front = GetStemDirection(a);
  ...
  const int stem_node_amount =
      static_cast<int>(glm::max(4.0f, stem_length / maize_layer->vertical_subdivision_length));
...
bool MaizeGrowthStages::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  ...
  if (ImGui::TreeNodeEx("States", ImGuiTreeNodeFlags_DefaultOpen)) {
    const float start_time = maize_growth_stages.empty() ? 1.0f : maize_growth_stages.begin()->first;
    if (start_time >= 0.01f) {
      if (ImGui::Button("New start state")) {
        ...
        if (maize_growth_stages.empty()) {
          ...
          Add(0.0f, maize_growth_stages.begin()->second);
        }
      }
    }
    ...
    for (auto it = maize_growth_stages.begin(); it != maize_growth_stages.end(); ++it) {
      if (ImGui::TreeNodeEx(("State " + std::to_string(state_index) + ": " + it->second.name).c_str())) {
        const std::string tag = "##MaizeState" + std::to_string(state_index);
    ...
    if (!maize_growth_stages.empty()) {
      if (ImGui::Button("New end state")) {
        changed = true;
        const float end_time = (--maize_growth_stages.end())->first;
        Add(end_time + 0.01f, (--maize_growth_stages.end())->second);
      }
      ImGui::SameLine();
      if (ImGui::Button("Remove end state")) {
        changed = true;
        maize_growth_stages.erase(--maize_growth_stages.end());
      }
    }
    ImGui::TreePop();
  }
...
void MaizeGrowthStages::Serialize(YAML::Emitter& out) const {
  out << YAML::Key << "state_mode" << YAML::Value << state_mode;
  out << YAML::Key << "maize_growth_stages" << YAML::Value << YAML::BeginSeq;
  for (auto& state : maize_growth_stages) {
    out << YAML::BeginMap;
...
void MaizeGrowthStages::Deserialize(const YAML::Node& in) {
  if (in["state_mode"])
    state_mode = in["state_mode"].as<int>();
  if (in["maize_growth_stages"]) {
    maize_growth_stages.clear();
    for (const auto& in_state : in["maize_growth_stages"]) {
      MaizeState state;
      state.Deserialize(in_state);
      maize_growth_stages.emplace_back(in_state["Time"].as<float>(), state);
    }
  }
}

std::shared_ptr<Texture2D> MaizeGrowthStages::GenerateThumbnailTexture() {
  ...
  if (!thumbnail) {
    ...
    thumbnail->Import(std::filesystem::absolute(std::filesystem::path("./DigitalAgricultureResources") /
                                                "Icons/SorghumGrowthStages.png")); // Reuse icon
  }
  return thumbnail;
}

Entity MaizeGrowthStages::CreateEntity(const float time) const {
  const auto scene = Application::GetActiveScene();
  const auto entity = scene->CreateEntity(GetTitle());
  const auto maize = scene->GetOrSetPrivateComponent<Maize>(entity).lock();
  const auto maize_state = AssetManager::CreateTemporaryAsset<MaizeDescriptor>();
  Apply(maize_state, time);
  maize->maize_descriptor = maize_state;
  maize->maize_growth_stages = GetSelf();
  maize->GenerateGeometryEntities(MaizeMeshGeneratorSettings{});
  return entity;
}

void MaizeGrowthStages::Add(float time, const MaizeState& state) {
  for (auto it = maize_growth_stages.begin(); it != maize_growth_stages.end(); ++it) {
    if (it->first == time) {
      it->second = state;
      return;
    ...
    if (it->first > time) {
      maize_growth_stages.insert(it, {time, state});
      return;
    }
  }
  maize_growth_stages.emplace_back(time, state);
  maize_growth_stages.back().second.name = "Unnamed";
}

void MaizeGrowthStages::ResetTime(const float previous_time, const float new_time) {
  for (auto& i : maize_growth_stages) {
...
void MaizeGrowthStages::Remove(const float time) {
  for (auto it = maize_growth_stages.begin(); it != maize_growth_stages.end(); ++it) {
    if (it->first == time) {
      maize_growth_stages.erase(it);
      return;
    }
  }
}
float MaizeGrowthStages::GetCurrentStartTime() const {
  if (maize_growth_stages.empty()) {
    return 0.0f;
  }
  return maize_growth_stages.begin()->first;
}
float MaizeGrowthStages::GetCurrentEndTime() const {
  if (maize_growth_stages.empty()) {
    return 0.0f;
  }
  return (--maize_growth_stages.end())->first;
}

bool MaizeGrowthStages::ImportCsv(const std::filesystem::path& file_path) {
  try {
    ...
    maize_growth_stages.resize(current_index);
    for (int row = 0; row < time_points.size(); row++) {
      int state_index = column_indices.at(time_points[row]).first;
      auto& state_pair = maize_growth_stages[state_index];
    ...
    for (auto& maize_state : maize_growth_stages) {
      maize_state.second.saved = false;
      int current_leaf_index = 0;
      for (auto& leaf_state : maize_state.second.leaves) {
        leaf_state.saved = false;
        leaf_state.index = current_leaf_index;
        current_leaf_index++;
      }
      maize_state.second.stem.saved = false;
      maize_state.second.panicle.saved = false;
    }
    saved_ = false;
