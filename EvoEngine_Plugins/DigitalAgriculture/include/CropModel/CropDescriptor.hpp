...
class CropDescriptor : public IAsset {
 public:
  ...
  float stem_elongation_gdd = 400.0f;     ///< Cumulative GDD at which stem elongation begins.
  float flowering_gdd = 800.0f;           ///< Cumulative GDD at which flowering begins.
  float grain_filling_gdd = 1000.0f;      ///< Cumulative GDD at which grain filling begins.
  float maturity_gdd = 1500.0f;           ///< Cumulative GDD at which the plant is physiologically mature.
  ...
  // ------------------------------------------------------------------
  // Panicle / ear (optional — not all crops have a panicle)
  // ------------------------------------------------------------------
