...
bool ParseFloatData(const std::string &fileName, int &numOfRows, int &numOfCols, float &minValue, float &maxValue,
...
bool ParseIntData(const std::string &fileName, int &numOfRows, int &numOfCols, int &minValue, int &maxValue,
                  std::vector<int> &data) {
  FILE *fp;
  if ((fp = fopen(fileName.c_str(), "r")) == nullptr) {
    EVOENGINE_ERROR("Error");
...
#pragma region material info
  ...
  // First save the info about BTFbase: name, materials saved, and how saved
  if (fscanf(fp, "%s\n%d\n%d\n%d\n%d\n%d\n", &line[0], &loadMaterials, &maxMaterials, &flagAllMaterials,
             &flagUse34DviewRep, &flagUsePDF2compactRep) != 6) {
    fclose(fp);
  ...
  int useCosBetaFlag, stepsPerBeta, tmp3, stepsPerAlpha, tmp5, stepsPerTheta, tmp7, stepsPerPhi, tmp9, tmp10, tmp11,
      tmp12;
  if (fscanf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d\n", &useCosBetaFlag, &stepsPerBeta, &tmp3, &stepsPerAlpha, &tmp5,
             &stepsPerTheta, &tmp7, &stepsPerPhi, &tmp9, &tmp10, &tmp11, &tmp12) != 12) {
    fclose(fp);
...
#pragma region Create shared variables
  ...
  m_bTFBase.m_sharedCoordinates.Set(tmp12, m_bTFBase.m_useCosBeta, m_bTFBase.m_numOfBeta, m_bTFBase.m_numOfAlpha,
                                    m_bTFBase.m_stepAlpha, m_bTFBase.m_numOfTheta, m_bTFBase.m_stepTheta,
                                    m_bTFBase.m_numOfPhi, m_bTFBase.m_stepPhi);
...
#pragma region Load material
  ...
  char nameM[200];
  if (fscanf(fp, "%s %s %s %s %d %d %d %d %f\n", &(nameM[0]), l1, l2, l3, &ro, &co, &pr, &pc, &hdrValue) != 9) {
  ...
  if (strcmp(nameM, materialName.c_str()) != 0) {
    EVOENGINE_ERROR("Some problem material name in file=" + std::string(nameM) + " other name=" + materialName + "\n");
    return false;
  }
...
#pragma region Load Data
  ...
  ParseIntData(prefix + "_PDF6Dslices.txt", m_bTFBase.m_pdf6.m_numOfRows, m_bTFBase.m_pdf6.m_numOfCols, minIntVal,
               maxIntVal, m_pdf6DSlices);
  ParseFloatData(prefix + "_PDF6Dscale.txt", m_bTFBase.m_pdf6.m_numOfRows, m_bTFBase.m_pdf6.m_numOfCols, minFloatVal,
                 maxFloatVal, m_pdf6DScales);

  prefix = materialDirectoryPath + "/" + "all";

  ParseFloatData(prefix + "_PDF1Dslice.txt", pdf1.m_numOfPdf1D, pdf1.m_numOfBeta, minFloatVal, maxFloatVal,
                 m_pdf1DBasis);

  ParseFloatData(prefix + "_colors.txt", ab.m_numOfColors, ab.m_numOfChannels, minFloatVal, maxFloatVal,
                 m_vectorColorBasis);

  ParseIntData(prefix + "_indexAB.txt", iab.m_numOfIndexSlices, iab.m_numOfBeta, minIntVal, maxIntVal, m_indexAbBasis);

  ParseIntData(prefix + "_PDF2Dcolours.txt", pdf2.m_color.m_numOfPdf2D, pdf2.m_color.m_numOfAlpha, minIntVal, maxIntVal,
               m_pdf2DColors);

  ParseIntData(prefix + "_PDF2Dslices.txt", pdf2.m_luminance.m_numOfPdf2D, pdf2.m_luminance.m_numOfAlpha, minIntVal,
               maxIntVal, m_pdf2DSlices);
  ParseFloatData(prefix + "_PDF2Dscale.txt", pdf2.m_luminance.m_numOfPdf2D, pdf2.m_luminance.m_numOfAlpha, minFloatVal,
                 maxFloatVal, m_pdf2DScales);

  ParseIntData(prefix + "_PDF2Dindices.txt", pdf2.m_numOfPdf2D, pdf2.m_lengthOfSlice, minIntVal, maxIntVal,
               m_indexLuminanceColors);

  ParseFloatData(prefix + "_PDF3Dscale.txt", pdf3.m_numOfPdf3D, pdf3.m_numOfTheta, minFloatVal, maxFloatVal,
                 m_pdf3DScales);

  ParseIntData(prefix + "_PDF3Dslices.txt", pdf3.m_numOfPdf3D, pdf3.m_numOfTheta, minIntVal, maxIntVal, m_pdf3DSlices);

  ParseFloatData(prefix + "_PDF4Dscale.txt", pdf4.m_numOfPdf4D, pdf4.m_numOfPhi, minFloatVal, maxFloatVal,
                 m_pdf4DScales);

  ParseIntData(prefix + "_PDF4Dslices.txt", pdf4.m_numOfPdf4D, pdf4.m_numOfPhi, minIntVal, maxIntVal, m_pdf4DSlices);
...
void DeserializeSharedCoordinates(SharedCoordinates &target, const YAML::Node &in) {
  if (in["m_useCosBeta"])
    target.m_useCosBeta = in["m_useCosBeta"].as<bool>();

  if (in["m_numOfBeta"])
    target.m_numOfBeta = in["m_numOfBeta"].as<int>();
  if (in["m_stepAlpha"])
    target.m_stepAlpha = in["m_stepAlpha"].as<float>();
  if (in["m_numOfAlpha"])
    target.m_numOfAlpha = in["m_numOfAlpha"].as<int>();
  if (in["m_stepTheta"])
    target.m_stepTheta = in["m_stepTheta"].as<float>();
  if (in["m_numOfTheta"])
    target.m_numOfTheta = in["m_numOfTheta"].as<int>();
  if (in["m_stepPhi"])
    target.m_stepPhi = in["m_stepPhi"].as<float>();
  if (in["m_numOfPhi"])
    target.m_numOfPhi = in["m_numOfPhi"].as<int>();

  if (in["m_beta"])
    target.m_beta = in["m_beta"].as<float>();
  if (in["m_alpha"])
    target.m_alpha = in["m_alpha"].as<float>();
  if (in["m_theta"])
    target.m_theta = in["m_theta"].as<float>();
  if (in["m_phi"])
    target.m_phi = in["m_phi"].as<float>();

  if (in["m_currentBetaLowBound"])
    target.m_currentBetaLowBound = in["m_currentBetaLowBound"].as<int>();
  if (in["m_weightBeta"])
    target.m_weightBeta = in["m_weightBeta"].as<float>();
  if (in["m_wMinBeta2"])
    target.m_wMinBeta2 = in["m_wMinBeta2"].as<float>();

  if (in["m_currentAlphaLowBound"])
    target.m_currentAlphaLowBound = in["m_currentAlphaLowBound"].as<int>();
  if (in["m_weightAlpha"])
    target.m_weightAlpha = in["m_weightAlpha"].as<float>();
  if (in["m_wMinAlpha2"])
    target.m_wMinAlpha2 = in["m_wMinAlpha2"].as<float>();

  if (in["m_currentThetaLowBound"])
    target.m_currentThetaLowBound = in["m_currentThetaLowBound"].as<int>();
  if (in["m_weightTheta"])
    target.m_weightTheta = in["m_weightTheta"].as<float>();
  if (in["m_wMinTheta2"])
    target.m_wMinTheta2 = in["m_wMinTheta2"].as<float>();

  if (in["m_currentPhiLowBound"])
    target.m_currentPhiLowBound = in["m_currentPhiLowBound"].as<int>();
  if (in["m_weightPhi"])
    target.m_weightPhi = in["m_weightPhi"].as<float>();

  if (in["m_scale"])
    target.m_scale = in["m_scale"].as<float>();

  if (in["m_hdrFlag"])
    target.m_hdrFlag = in["m_hdrFlag"].as<bool>();
  if (in["m_codeBtfFlag"])
    target.m_codeBtfFlag = in["m_codeBtfFlag"].as<bool>();
}
...
void DeserializeVectorColor(VectorColor &target, const YAML::Node &in) {
  if (in["m_startIndex"])
    target.m_startIndex = in["m_startIndex"].as<int>();
  if (in["m_numOfChannels"])
    target.m_numOfChannels = in["m_numOfChannels"].as<int>();
  if (in["m_numOfColors"])
    target.m_numOfColors = in["m_numOfColors"].as<int>();
}
...
void DeserializeIndexAB(IndexAB &target, const YAML::Node &in) {
  if (in["m_numOfIndexSlices"])
    target.m_numOfIndexSlices = in["m_numOfIndexSlices"].as<int>();
  if (in["m_numOfBeta"])
    target.m_numOfBeta = in["m_numOfBeta"].as<int>();
  if (in["m_ab"])
    DeserializeVectorColor(target.m_ab, in["m_ab"]);
}
...
void DeserializePDF1D(PDF1D &target, const YAML::Node &in) {
  if (in["m_numOfBeta"])
    target.m_numOfBeta = in["m_numOfBeta"].as<int>();
  if (in["m_numOfPdf1D"])
    target.m_numOfPdf1D = in["m_numOfPdf1D"].as<int>();
}
...
void DeserializePDF2D(PDF2D &target, const YAML::Node &in) {
  if (in["m_numOfPdf2D"])
    target.m_numOfPdf2D = in["m_numOfPdf2D"].as<int>();
  if (in["m_size2D"])
    target.m_size2D = in["m_size2D"].as<int>();
  if (in["m_lengthOfSlice"])
    target.m_lengthOfSlice = in["m_lengthOfSlice"].as<int>();

  if (in["m_color.m_numOfPdf2D"])
    target.m_color.m_numOfPdf2D = in["m_color.m_numOfPdf2D"].as<int>();
  if (in["m_color.m_numOfAlpha"])
    target.m_color.m_numOfAlpha = in["m_color.m_numOfAlpha"].as<int>();
  if (in["m_color.m_size2D"])
    target.m_color.m_size2D = in["m_color.m_size2D"].as<int>();

  if (in["m_luminance.m_numOfPdf2D"])
    target.m_luminance.m_numOfPdf2D = in["m_luminance.m_numOfPdf2D"].as<int>();
  if (in["m_luminance.m_numOfAlpha"])
    target.m_luminance.m_numOfAlpha = in["m_luminance.m_numOfAlpha"].as<int>();

  if (in["m_iab"])
    DeserializeIndexAB(target.m_iab, in["m_iab"]);
  if (in["m_pdf1"])
    DeserializePDF1D(target.m_pdf1, in["m_pdf1"]);
}
...
void DeserializePDF3D(PDF3D<T> &target, const YAML::Node &in) {
  if (in["m_numOfPdf3D"])
    target.m_numOfPdf3D = in["m_numOfPdf3D"].as<int>();
  if (in["m_numOfTheta"])
    target.m_numOfTheta = in["m_numOfTheta"].as<int>();

  if (in["m_pdf2"])
    DeserializePDF2D(target.m_pdf2, in["m_pdf2"]);
}
...
void DeserializePDF4D(PDF4D<T> &target, const YAML::Node &in) {
  if (in["m_numOfPdf4D"])
    target.m_numOfPdf4D = in["m_numOfPdf4D"].as<int>();
  if (in["m_numOfPhi"])
    target.m_numOfPhi = in["m_numOfPhi"].as<int>();
  if (in["m_stepPhi"])
    target.m_stepPhi = in["m_stepPhi"].as<float>();
  ...
  if (in["m_pdf3"])
    DeserializePDF3D(target.m_pdf3, in["m_pdf3"]);
}
...
void DeserializePDF6D(PDF6D<T> &target, const YAML::Node &in) {
  if (in["m_numOfRows"])
    target.m_numOfRows = in["m_numOfRows"].as<int>();
  if (in["m_numOfCols"])
    target.m_numOfCols = in["m_numOfCols"].as<int>();
  if (in["m_rowsOffset"])
    target.m_rowsOffset = in["m_rowsOffset"].as<int>();

  if (in["m_colsOffset"])
    target.m_colsOffset = in["m_colsOffset"].as<int>();

  if (in["m_colorAmount"])
    target.m_colorAmount = in["m_colorAmount"].as<int>();

  if (in["m_pdf4"])
    DeserializePDF4D(target.m_pdf4, in["m_pdf4"]);
}
...
void DeserializeBTFBase(BTFBase &target, const YAML::Node &in) {
  if (in["m_sharedCoordinates"])
    DeserializeSharedCoordinates(target.m_sharedCoordinates, in["m_sharedCoordinates"]);
  if (in["m_pdf6"])
    DeserializePDF6D(target.m_pdf6, in["m_pdf6"]);

  if (in["m_hdr"])
    target.m_hdr = in["m_hdr"].as<bool>();
  if (in["m_hdrValue"])
    target.m_hdrValue = in["m_hdrValue"].as<float>();

  if (in["m_multiplier"])
    target.m_multiplier = in["m_multiplier"].as<float>();
  if (in["m_texCoordMultiplier"])
    target.m_texCoordMultiplier = in["m_texCoordMultiplier"].as<float>();
  if (in["m_gamma"])
    target.m_gamma = in["m_gamma"].as<float>();

  if (in["m_materialOrder"])
    target.m_materialOrder = in["m_materialOrder"].as<int>();
  if (in["m_nColor"])
    target.m_nColor = in["m_nColor"].as<int>();

  if (in["m_useCosBeta"])
    target.m_useCosBeta = in["m_useCosBeta"].as<bool>();
  if (in["m_mPostScale"])
    target.m_mPostScale = in["m_mPostScale"].as<float>();
  ...
  if (in["m_allMaterialsInOneDatabase"])
    target.m_allMaterialsInOneDatabase = in["m_allMaterialsInOneDatabase"].as<bool>();
  if (in["m_use34ViewRepresentation"])
    target.m_use34ViewRepresentation = in["m_use34ViewRepresentation"].as<bool>();
  if (in["m_usePdf2CompactRep"])
    target.m_usePdf2CompactRep = in["m_usePdf2CompactRep"].as<bool>();

  if (in["m_materialCount"])
    target.m_materialCount = in["m_materialCount"].as<int>();
}
...
void CompressedBTF::UploadDeviceData() {
  ...
  pdf6.m_devicePdf6DSlices = reinterpret_cast<int *>(pdf6.m_pdf6DSlicesBuffer.DevicePointer());
  pdf6.m_devicePdf6DScales = reinterpret_cast<float *>(pdf6.m_pdf6DScalesBuffer.DevicePointer());

  auto &pdf4 = pdf6.m_pdf4;
  pdf4.m_pdf4DScalesBuffer.Upload(m_pdf4DScales);
  pdf4.m_pdf4DSlicesBuffer.Upload(m_pdf4DSlices);
  pdf4.m_devicePdf4DScales = reinterpret_cast<float *>(pdf4.m_pdf4DScalesBuffer.DevicePointer());
  pdf4.m_devicePdf4DSlices = reinterpret_cast<int *>(pdf4.m_pdf4DSlicesBuffer.DevicePointer());

  auto &pdf3 = pdf4.m_pdf3;
  pdf3.m_pdf3DScalesBuffer.Upload(m_pdf3DScales);
  pdf3.m_pdf3DSlicesBuffer.Upload(m_pdf3DSlices);
  pdf3.m_devicePdf3DScales = reinterpret_cast<float *>(pdf3.m_pdf3DScalesBuffer.DevicePointer());
  pdf3.m_devicePdf3DSlices = reinterpret_cast<int *>(pdf3.m_pdf3DSlicesBuffer.DevicePointer());

  auto &pdf2 = pdf3.m_pdf2;
  pdf2.m_indexLuminanceColorBuffer.Upload(m_indexLuminanceColors);
  pdf2.m_deviceIndexLuminanceColor = reinterpret_cast<int *>(pdf2.m_indexLuminanceColorBuffer.DevicePointer());

  auto &color = pdf2.m_color;
  color.m_pdf2DColorsBuffer.Upload(m_pdf2DColors);
  color.m_devicePdf2DColors = reinterpret_cast<int *>(color.m_pdf2DColorsBuffer.DevicePointer());
  ...
  luminance.m_devicePdf2DSlices = reinterpret_cast<int *>(luminance.m_pdf2DSlicesBuffer.DevicePointer());
  luminance.m_devicePdf2DScales = reinterpret_cast<float *>(luminance.m_pdf2DScalesBuffer.DevicePointer());

  auto &iab = pdf2.m_iab;
  iab.m_indexAbBasisBuffer.Upload(m_indexAbBasis);
  iab.m_deviceIndexAbBasis = reinterpret_cast<int *>(iab.m_indexAbBasisBuffer.DevicePointer());

  auto &pdf1 = pdf2.m_pdf1;
  pdf1.m_pdf1DBasisBuffer.Upload(m_pdf1DBasis);
  pdf1.m_devicePdf1DBasis = reinterpret_cast<float *>(pdf1.m_pdf1DBasisBuffer.DevicePointer());

  auto &ab = iab.m_ab;
  ab.m_vectorColorBasisBuffer.Upload(m_vectorColorBasis);
  ab.m_deviceVectorColorBasis = reinterpret_cast<float *>(ab.m_vectorColorBasisBuffer.DevicePointer());
