...
struct PDF6D {
  ...
  void Init(const int &numOfRows, const int &numOfCols, const int &rowsOffset, const int &colsOffset,
            const int &colorAmount) {
...
#pragma region CUDA
  ...
  __device__ void GetValDeg2(const glm::vec2 &texCoord, float illuminationTheta, float illuminationPhi, float viewTheta,
                             float viewPhi, T &out, SharedCoordinates &tc, const bool &print) const {
    ...
    ConvertThetaPhiToBetaAlpha(glm::radians(illuminationTheta), glm::radians(illuminationPhi), tc.m_beta, tc.m_alpha,
                               tc);
    ...
    if (glm::isnan(tc.m_beta) || glm::isnan(tc.m_alpha) || glm::isnan(viewTheta) || glm::isnan(viewPhi)) {
      if (print)
        printf("Value is nan! beta[%.2f] alpha[%.2f] theta[%.2f] phi[%.2f]", tc.m_beta, tc.m_alpha, viewTheta, viewPhi);
      return;
    }
