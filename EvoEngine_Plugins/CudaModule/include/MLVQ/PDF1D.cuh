...
#pragma region CUDA
  ...
  __device__ float GetVal(const int &sliceIndex, SharedCoordinates &tc) const {
    ...
    return (1.f - tc.wBeta) * PDF1Dbasis[sliceIndex][tc.iBeta] + tc.wBeta * PDF1Dbasis[sliceIndex][tc.iBeta + 1];
#endif
    ...
    const float p0 = m_devicePdf1DBasis[sliceIndex * m_numOfBeta + tc.m_currentBetaLowBound];
    const float p1 = m_devicePdf1DBasis[sliceIndex * m_numOfBeta + tc.m_currentBetaLowBound + 1];
    float m0h, m1h;
    if (tc.m_currentBetaLowBound == 0)
      m0h = p1 - p0;  // end point
    else
      // standard way
      m0h = 0.5f * (p1 - m_devicePdf1DBasis[sliceIndex * m_numOfBeta + tc.m_currentBetaLowBound - 1]);
    ...
    if (tc.m_currentBetaLowBound == m_numOfBeta - 2)
      ...
      // standard way
      m1h = 0.5f * (m_devicePdf1DBasis[sliceIndex * m_numOfBeta + tc.m_currentBetaLowBound + 1] - p0);
    const float t2 = w * w;
    const float t3 = t2 * w;
