...
__device__ inline void YCbCrToRgb(const glm::vec3 &yCbCr, glm::vec3 &rgb) {
  ...
  // G
  rgb[1] = yCbCr[0] * 1.1643828f - yCbCr[1] * 0.39176172f - yCbCr[2] * 0.81296875f + 135.576f;
  // B
...
/*! YCbCr to RGB space */
__device__ inline void YCbCrToRgbNormalized(const glm::vec3 &yCbCr, glm::vec3 &rgb) {
  assert(yCbCr[0] >= 0.f);
  assert(yCbCr[1] >= 0.f);
  assert(yCbCr[2] >= 0.f);
  ...
  // G
  rgb[1] = yCbCr[0] * 1.1643828f - yCbCr[1] * 0.39176172f - yCbCr[2] * 0.81296875f + 135.576f / 256.0f;
  // B
...
__device__ inline int LogLuv2RgbNormalized(const glm::vec3 &luv, glm::vec3 &rgb) {
  /*! \brief Conversion from LogLu'v' (not L*u*v*)  colour space according to
    [Ward98] into RGB colourspace for Observer. = 2degree, Illuminant = D65 */
