...
#pragma region CUDA
    ...
    __device__ void GetVal(const int &pdf2DIndex, glm::vec3 &out, SharedCoordinates &tc, const IndexAB &iab,
...
#pragma region CUDA
    ...
    // float* m_pdf2DNorm;
    __device__ void GetVal(const int &pdf2DIndex, glm::vec3 &out, SharedCoordinates &tc, const PDF1D &pdf1,
                           const bool &print) const {
      assert(pdf2DIndex >= 0 && pdf2DIndex < m_numOfPdf2D);
      const int i = tc.m_currentAlphaLowBound;
      const float w = tc.m_weightAlpha;
      ...
      // luminances
      const float l1 = m_devicePdf2DScales[pdf2DIndex * m_numOfAlpha + i] *
                       pdf1.GetVal(m_devicePdf2DSlices[pdf2DIndex * m_numOfAlpha + i], tc);
      const float l2 = m_devicePdf2DScales[pdf2DIndex * m_numOfAlpha + i + 1] *
...
#pragma region CUDA
  ...
  __device__ void GetVal(const int &pdf2DIndex, glm::vec3 &out, SharedCoordinates &tc, const bool &print) const {
    assert(pdf2DIndex >= 0 && pdf2DIndex < m_numOfPdf2D);
    ...
    m_color.GetVal(m_deviceIndexLuminanceColor[pdf2DIndex * m_lengthOfSlice + 1], userCMdata, tc, m_iab, print);
    if (print)
      printf("Sampling from Luminance...");
    m_luminance.GetVal(m_deviceIndexLuminanceColor[pdf2DIndex * m_lengthOfSlice + 0], userCMdata, tc, m_pdf1, print);
