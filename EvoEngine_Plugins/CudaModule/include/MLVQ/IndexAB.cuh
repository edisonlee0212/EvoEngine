...
#pragma region CUDA
  ...
  // (0,1)
  __device__ float Get(const int &sliceIndex, const int &posBeta, const int &posAB, SharedCoordinates &tc) const {
    assert(sliceIndex >= 0 && sliceIndex < m_numOfIndexSlices);
    assert(posBeta >= 0 && posBeta < m_numOfBeta);
    return m_ab.Get(m_deviceIndexAbBasis[sliceIndex * m_numOfBeta + posBeta], posAB, tc);
  }

  // Here beta is specified by 'tc'
  __device__ void GetVal(const int &sliceIndex, glm::vec3 &out, SharedCoordinates &tc) const {
