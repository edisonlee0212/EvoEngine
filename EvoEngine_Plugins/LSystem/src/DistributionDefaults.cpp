...
void SetCurveLinearRange(Curve2D& curve,
                         const float y_start,
                         const float y_end,
...
void DistributionDefaults::SetCurveFlat(Curve2D& curve,
                                        const float y_value,
                                        const int sample_count) {
...
void DistributionDefaults::ApplyLinearGrowthCurveDefaults(PlottedDistribution<float>& distribution,
                                                          const float mean_start,
                                                          const float mean_end,
                                                          const int sample_count) {
...
void DistributionDefaults::ApplySingleDefaults(SingleDistribution<float>& distribution,
                                               const float mean,
...
SingleDistribution<float> DistributionDefaults::MakeSingleDefaults(const float mean,
                                                                   const float deviation) {
...
evo_engine::PlottedDistributionSettings DistributionDefaults::MakePlottedGuiSettings(
    const std::string& tip) {
  evo_engine::PlottedDistributionSettings settings;
  settings.tip = tip;
  settings.mean_settings.m_tip =
      "Mean response curve. x is normalized axis [0, 1].";
  settings.dev_settings.m_tip =
      "Standard deviation (sigma) curve. Zero keeps deterministic behavior.";
  return settings;
}

bool DistributionDefaults::InspectPlottedDistributionCategory(
    const char* category_label,
    const std::initializer_list<PlottedDistributionUiEntry> entries,
