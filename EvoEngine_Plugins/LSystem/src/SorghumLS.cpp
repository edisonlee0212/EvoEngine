...
std::atomic<bool> g_sorghum_leaf_two_sided_default{true};
...
bool BuildStemContextForLeafNode(const SorghumGraph& graph,
  ...
  auto cursor = graph.PeekNode(leaf_handle).GetParentHandle();
  while (cursor >= 0) {
...
void SorghumLS::GrowToTargetGDD(const bool uncapped_growth) {
...
void SorghumLS::RebuildGeometry() {
  ...
  // -------------------------------------------------------------------------
  {
    ...
    for (const auto handle : sorted) {
      ...
      if (color_mode == ColorMode::ByNode) {
        ...
        pi.instance_color = glm::vec4(0.28f, 0.70f, 0.24f, 1.0f);
      } else {
        pi.instance_color = glm::vec4(0.36f, 0.58f, 0.24f, 1.0f);
      }
      infos.push_back(pi);
    }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    for (const auto handle : sorted) {
      ...
      // Optional color overrides for debug modes; LeafSenescence keeps
      // the generated green->brown gradient from SorghumLeafMesh.
      if (color_mode != ColorMode::LeafSenescence) {
        ...
        } else if (color_mode == ColorMode::ByNode) {
          override_color = HashToColor(static_cast<uint32_t>(node.GetIndex()));
        }
...
bool SorghumLS::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  bool changed = false;
  ...
  if (ImGui::TreeNode("Leaf meshing")) {
    ...
    if (ImGui::Checkbox("Leaf bottom face", &leaf_bottom_face)) {
      g_sorghum_leaf_two_sided_default.store(leaf_bottom_face, std::memory_order_relaxed);
      changed = true;
    }
    ImGui::TreePop();
  }
...
void SorghumLS::Serialize(YAML::Emitter& out) const {
...
void SorghumLS::Deserialize(const YAML::Node& in) {
  ...
  // Keep the process-wide fallback synced with latest deserialized value.
  g_sorghum_leaf_two_sided_default.store(leaf_bottom_face, std::memory_order_relaxed);
}
