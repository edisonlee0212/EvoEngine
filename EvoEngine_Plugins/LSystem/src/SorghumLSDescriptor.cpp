...
void ClampDescriptorValues(SorghumLSDescriptor& descriptor) {
  ...
  ClampPlottedDistributionRange(descriptor.leaf_curling, -180.0f, 180.0f, 180.0f);
...
bool SorghumLSDescriptor::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  bool changed = false;
  ...
  const auto inspect_plotted_distribution = [](const char* label,
                                               evo_engine::PlottedDistribution<float>& distribution) {
    return distribution.OnInspect(label, DistributionDefaults::MakePlottedGuiSettings());
  };
  ...
  if (ImGui::TreeNodeEx("Culm Topology", ImGuiTreeNodeFlags_DefaultOpen)) {
    changed |= total_phytomer_count.OnInspect("Total Phytomer Count", 0.5f);
    changed |= phyllotaxis_angle.OnInspect("Phyllotaxis Angle (deg)", 1.0f);
    changed |= branch_azimuth_offset.OnInspect("Branch Azimuth Offset (deg)", 0.5f);
    ImGui::TreePop();
  }
  ...
  if (ImGui::TreeNodeEx("Leaf Morphology", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    changed |= inspect_plotted_distribution("Blade Max Width (Rank)", leaf_blade_max_width);
    ...
    changed |= inspect_plotted_distribution("Leaf Curling (Rank)", leaf_curling);
    ...
    changed |= leaf_waviness_frequency.OnInspect("Waviness Frequency", 0.1f);
    ImGui::TreePop();
  }

  if (ImGui::TreeNodeEx("Leaf Lifecycle", ImGuiTreeNodeFlags_DefaultOpen)) {
    changed |= leaf_lifespan_years.OnInspect("Leaf Lifespan (years)", 0.05f);
    changed |= leaf_wilting_years.OnInspect("Leaf Wilting Duration (years)", 0.02f);
    ImGui::TreePop();
  }

  if (ImGui::TreeNodeEx("Tillering", ImGuiTreeNodeFlags_DefaultOpen)) {
    changed |= tiller_count.OnInspect("Tiller Count", 0.5f);
    changed |= inspect_plotted_distribution("Tiller Initiation Delay (GDD)", tiller_initiation_delay_gdd);
    changed |= tiller_insertion_angle.OnInspect("Tiller Insertion Angle", 0.5f);
    changed |= tiller_phytomer_count_scale.OnInspect("Tiller Phytomer Count Scale", 0.01f);
    changed |= tiller_thickness_ratio.OnInspect("Tiller Thickness Ratio", 0.01f);
    ImGui::TreePop();
  }

  if (ImGui::TreeNodeEx("Thermal Development", ImGuiTreeNodeFlags_DefaultOpen)) {
    changed |= target_gdd.OnInspect("Target GDD", 5.0f);
    changed |= gdd_per_day.OnInspect("Thermal GDD/day", 0.25f);
    changed |= plastochron_gdd.OnInspect("Plastochron GDD", 1.0f);
    changed |= maturity_gdd.OnInspect("Maturity GDD", 5.0f);
    changed |= main_axis_plastochron_scale.OnInspect("Main-Axis Plastochron Scale", 0.01f);
    changed |= lateral_axis_plastochron_scale.OnInspect("Lateral-Axis Plastochron Scale", 0.01f);
    changed |= lateral_bud_plastochron_scale.OnInspect("Lateral-Bud Plastochron Scale", 0.01f);
    changed |= maturity_initiation_coupling.OnInspect("Maturity->Initiation Coupling", 0.01f);
    changed |= reference_maturity_gdd.OnInspect("Reference Maturity GDD", 5.0f);
    ImGui::TreePop();
  }
  ...
  if (ImGui::TreeNodeEx("Growth Curves", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    changed |= DistributionDefaults::InspectPlottedDistributionCategory(
        ...
        {
            {"Leaf Blade Length", &leaf_blade_growth_curve,
             "Controls normalized leaf blade elongation over thermal age."},
            {"Leaf Width", &leaf_width_growth_curve,
             "Controls normalized leaf width expansion over thermal age."},
            ...
            {"Leaf Curling", &leaf_curling_development_curve,
             "Controls normalized curling development over thermal age."},
    ...
    changed |= DistributionDefaults::InspectPlottedDistributionCategory(
        ...
        {
            {"Width Along Leaf", &width_along_leaf,
             "Profile of relative width from collar (x=0) to tip (x=1)."},
            {"Curling Along Leaf", &curling_along_leaf,
             "Profile of relative curling from collar (x=0) to tip (x=1)."},
...
void SorghumLSDescriptor::Serialize(YAML::Emitter& out) const {
  // Culm topology.
  total_phytomer_count.Save("total_phytomer_count", out);
  ...
  internode_thickness.Save("internode_thickness", out);
  leaf_blade_length.Save("leaf_blade_length", out);
  leaf_blade_max_width.Save("leaf_blade_max_width", out);
  leaf_sheath_length.Save("leaf_sheath_length", out);
  leaf_insertion_angle.Save("leaf_insertion_angle", out);
  ...
  // Leaf lifecycle.
  leaf_lifespan_years.Save("leaf_lifespan_years", out);
  leaf_wilting_years.Save("leaf_wilting_years", out);

  // Tillering.
  tiller_count.Save("tiller_count", out);
  tiller_initiation_delay_gdd.Save("tiller_initiation_delay_gdd", out);
  tiller_insertion_angle.Save("tiller_insertion_angle", out);
  tiller_phytomer_count_scale.Save("tiller_phytomer_count_scale", out);
  tiller_thickness_ratio.Save("tiller_thickness_ratio", out);

  // Thermal.
  target_gdd.Save("target_gdd", out);
  gdd_per_day.Save("gdd_per_day", out);
  plastochron_gdd.Save("plastochron_gdd", out);
  maturity_gdd.Save("maturity_gdd", out);
  ...
  // Growth curves.
  internode_elongation_curve.Save("internode_elongation_curve", out);
  internode_thickness_curve.Save("internode_thickness_curve", out);
  leaf_blade_growth_curve.Save("leaf_blade_growth_curve", out);
  leaf_width_growth_curve.Save("leaf_width_growth_curve", out);
  leaf_angle_development_curve.Save("leaf_angle_development_curve", out);
  leaf_curling_development_curve.Save("leaf_curling_development_curve", out);
  leaf_bending_development_curve.Save("leaf_bending_development_curve", out);
...
void SorghumLSDescriptor::Deserialize(const YAML::Node& in) {
...
void SorghumLSDescriptor::RegisterExplorableAxes(ParamSpaceExplorer& explorer) {
  ...
  explorer.AddPlotted("leaf_bending", "LBD", d.leaf_bending);
