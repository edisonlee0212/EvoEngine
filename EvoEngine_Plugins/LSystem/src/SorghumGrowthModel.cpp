...
void SorghumGrowthModel::Initialize(const SorghumLSDescriptor& descriptor,
  ...
  // Seed the single root apex (Pine-style axiom). Tiller buds are spawned
  // as branch successors when this apex emits its first phytomer, so the
  // engine alone manages the multi-node structure.
  graph = SorghumGraph(1);
  auto& root = graph.RefNode(0);
  root.symbol_id = SorghumSymbol::Apex;
  ...
  root.data.Set<SorghumApex>(apex);
...
void SorghumGrowthModel::UpdateNodeInfoImpl(LGraphNode<SorghumModuleData>& node) {
  ...
  // Apex / TillerBud / PanicleBud carry no axis geometry.
  node.info.length = 0.0f;
  node.info.thickness = 0.0f;
}
