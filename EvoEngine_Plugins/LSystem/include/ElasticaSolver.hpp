...
inline PlanarElasticaOutput SolvePlanarElastica(const PlanarElasticaInput& in) {
  ...
  if (in.length_m <= 0.0f) return out;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (in.intrinsic_curvature_per_m) kappa_intr[i] = in.intrinsic_curvature_per_m(s_norm);
    if (in.bending_stiffness_Pa_m4) EI[i] = std::max(1e-20f, in.bending_stiffness_Pa_m4(s_norm));
    if (in.mass_per_length_kg_m) mu[i] = std::max(0.0f, in.mass_per_length_kg_m(s_norm));
  }
  ...
  for (int iter = 0; iter < std::max(1, in.max_iterations); ++iter) {
    ...
    for (int i = 1; i < n; ++i) {
      ...
      out.positions_xz[i] = out.positions_xz[i - 1] +
                            ds * glm::vec2(std::sin(t_mid), std::cos(t_mid));
    }

    // (2) Compute bending moment at every station via a tip->base sweep.
    //     M(s_i) = F_tip x (r_tip - r_i)  +  Σ_{j > i} (mu_j * g_xz * ds) x (r_j - r_i)
    ...
    for (int i = 0; i < n; ++i) {
      ...
      for (int j = i + 1; j < n; ++j) {
        ...
        if (force.x == 0.0f && force.y == 0.0f) continue;
...
inline CantileverValidationResult RunCantileverValidation(const float length_m,
                                                          const float EI_Pa_m4,
                                                          const float tip_force_N,
                                                          const int station_count = 33) {
  PlanarElasticaInput in;
  in.length_m = length_m;
  in.station_count = station_count;
  in.bending_stiffness_Pa_m4 = [EI_Pa_m4](float) { return EI_Pa_m4; };
  in.intrinsic_curvature_per_m = [](float) { return 0.0f; };
  ...
  r.relative_error = (std::abs(r.analytic_tip_x_m) > 1e-12f)
                          ? std::abs(r.solver_tip_x_m - r.analytic_tip_x_m) /
