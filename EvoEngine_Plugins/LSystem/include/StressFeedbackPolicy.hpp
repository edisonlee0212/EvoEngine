...
//
// Phase 5 of the biologically-emergent organ growth substrate. Closes the
// cell-mechanics loop the brief calls for: bending stress extracted from the
// Phase 4 elastica solver feeds back into both the growth field
// (`g(s, side, t) <- g_baseline * feedback_growth(σ)`) and the material
// stiffness (`E(t) <- E_baseline * feedback_stiffness(σ_history)`).
//
// Default policies:
...
//   - σ history is exponentially-moving-averaged in time (alpha = 0 disables
//     all updates → policy never engages → preserves Phase 4 results bit-
//     exactly).
//   - per-station σ samples may be smoothed in arc length via a simple
...
struct StressFeedbackPolicy {
  ...
  /// Normalization stress (Pa). σ_norm = σ / sigma_reference_Pa. Order of
  /// magnitude for slender needle bending: 1e5 .. 1e6 Pa. Must be > 0 for
  /// the policy to be active.
  float sigma_reference_Pa = 1.0e6f;

  /// Exponential-moving-average rate per *rebuild tick*: σ_ema <- (1-α)·σ_ema + α·|σ|.
  /// α = 0 → EMA is never updated (policy effectively inert across rebuilds).
  /// α = 1 → no smoothing (instantaneous σ replaces history every tick).
  float time_ema_alpha = 0.0f;

  /// Arc-length smoothing window in *normalized* arc length [0,1] applied to
  /// the per-station σ samples *before* EMA blending. A simple box average
  ...
  /// at most 4× baseline regardless of stress history.
  ...
  /// When false, callers MUST short-circuit and use baseline EI / κ.
  [[nodiscard]] bool IsActive() const {
    if (sigma_reference_Pa <= 0.0f) return false;
    if (time_ema_alpha <= 0.0f) return false;
  ...
  /// `exp(-gain · σ_norm)` saturates smoothly to 0; the explicit clamp
  /// keeps geometry sane under unphysical σ spikes.
  [[nodiscard]] float feedback_growth(const float sigma_Pa) const {
    if (growth_suppression_gain == 0.0f) return 1.0f;
  ...
  /// sample. `1 + gain · σ_ema_norm` rises linearly; the explicit clamp
  /// caps unbounded hardening.
  [[nodiscard]] float feedback_stiffness(const float sigma_ema_Pa) const {
    if (stiffness_hardening_gain == 0.0f) return 1.0f;
  ...
  /// Update an EMA buffer in place from a fresh per-station |σ| sample
  ...
  void UpdateEma(const std::vector<float>& sigma_samples_Pa,
                 std::vector<float>& ema_buffer_Pa) const {
    ...
    if (arc_length_smoothing_window > 0.0f && N > 2) {
      ...
      const float half_window =
          0.5f * std::clamp(arc_length_smoothing_window, 0.0f, 1.0f);
      const int half_stations =
          std::max(1, static_cast<int>(std::round(half_window * static_cast<float>(N - 1))));
      for (std::size_t i = 0; i < N; ++i) {
        const int lo = std::max(0, static_cast<int>(i) - half_stations);
        const int hi = std::min(static_cast<int>(N) - 1,
                                static_cast<int>(i) + half_stations);
        float acc = 0.0f;
        int cnt = 0;
        for (int j = lo; j <= hi; ++j) {
