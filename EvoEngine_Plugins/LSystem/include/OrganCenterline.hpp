...
struct CenterlineSample {
  ...
  glm::vec3 binormal = glm::vec3(0, 1, 0); ///< tangent × normal; right-handed frame.
...
class OrganCenterline {
 public:
  ...
  CenterlineSample Sample(float s,
                          const glm::vec3& base_normal = glm::vec3(1, 0, 0)) const {
    EnsureArcLengthTable();
    ...
    while (lo + 1 < hi) {
      ...
      if (arc_length_table_[mid] <= s) lo = mid; else hi = mid;
    }
    ...
    out.normal = SafeNormalize(
        glm::mix(a.normal, b.normal, u) -
            out.tangent * glm::dot(glm::mix(a.normal, b.normal, u), out.tangent),
        a.normal);
    out.binormal = SafeNormalize(glm::cross(out.tangent, out.normal),
                                 glm::cross(a.tangent, a.normal));
    out.arc_length = s;
    return out;
  }
  ...
  void Invalidate() { arc_length_dirty_ = true; }
 ...
 private:
  ...
  static glm::vec3 SafeNormalize(const glm::vec3& v, const glm::vec3& fallback) {
    ...
    if (l2 > 1e-20f) return v * (1.0f / std::sqrt(l2));
    return fallback;
  }
  ...
  glm::vec3 EvaluatePolyline(float t) const {
    ...
    return CatmullRom(control_points_[i0], control_points_[i1],
                      control_points_[i2], control_points_[i3], u);
  }

  void EnsureArcLengthTable() const {
    if (!arc_length_dirty_) return;
    ...
    for (int i = 1; i < resample_count_; ++i) {
      arc_length_table_[i] = arc_length_table_[i - 1] +
                             glm::length(sampled_positions_[i] - sampled_positions_[i - 1]);
    }
    arc_length_dirty_ = false;
    frames_dirty_ = true;
  }
  ...
  void EnsureFrameTable(const glm::vec3& base_normal) const {
    if (!frames_dirty_ && glm::length(base_normal - cached_base_normal_) < 1e-6f) return;
    ...
    glm::vec3 prev_normal = base_normal -
                            prev_tangent * glm::dot(base_normal, prev_tangent);
