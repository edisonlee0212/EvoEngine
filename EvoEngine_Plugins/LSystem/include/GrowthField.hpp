...
//
//     κ_eq(s) = (g_abaxial(s) - g_adaxial(s)) / d(s)
//
...
// any organ that does not opt in produces κ ≡ 0 — preserving Phase 1/Phase 2
...
struct BilateralGrowthField1D {
  ...
  [[nodiscard]] bool IsInert() const {
    return diameter_m <= 0.0f ||
           (adaxial_bias == 0.0f && abaxial_bias == 0.0f &&
            gradient_per_arclen == 0.0f);
  }
...
inline OrganCenterline BuildBentNeedleCenterline(const float length,
                                                 const int segments,
  ...
  const float wave_amplitude_rad =
      (clamped_wave_amplitude_deg * (kPi / 180.0f)) * std::min(clamped_maturation, 1.0f);
  ...
  //   θ(0) = 0  (tangent along +Z)
  //   dθ/ds = κ(s_norm) * maturation_multiplier
  //   x(s)  = ∫ sin(θ) ds, z(s) = ∫ cos(θ) ds
  ...
  for (int i = 0; i < sub_steps; ++i) {
    ...
    float kappa = has_growth_field
        ? field.EquilibriumCurvature(s_norm) * clamped_maturation
        : 0.0f;
    if (has_wave) {
      ...
      const float kappa_wave =
          (wave_amplitude_rad * kTwoPi * clamped_wave_frequency * envelope * std::cos(phase)) /
          clamped_length;
      kappa += kappa_wave;
    }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    const int idx = std::clamp(
        static_cast<int>(std::round(u * static_cast<float>(sub_steps))), 0, sub_steps);
