...
class StrandGroup {
 ...
 public:
  ...
  void Subdivide(
      ...
      const std::function<void(StrandHandle src_handle, OSd& tgt_data)>& strand_data_action,
                 const std::function<void(float start_root_distance, float end_root_distance,
                                          StrandSegmentHandle src_handle, uint32_t segment_index,
                               float segment_t, OSsd& tgt_data, uint32_t sub_segment_index)>& segment_data_action,
      float tolerance) const;
...
void StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::CalculateRotations() {
  for (const auto& strand : strands_) {
    ...
    strand_segments_[strand.strand_segment_handles_[0]].rotation =
        glm::quatLookAt(front, glm::vec3(front.y, front.z, front.x));
...
StrandSegmentHandle StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::Extend(
  ...
  const auto new_segment_handle = AllocateStrandSegment(
      target_handle, prev_handle, static_cast<StrandSegmentHandle>(strand.strand_segment_handles_.size()));
...
StrandHandle StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::Cut(
  ...
  if (target_segment_handle == -1)
    return -1;
  if (const auto& handles = strands_[strand_segments_[target_segment_handle].strand_handle_].strand_segment_handles_;
      handles.front() == target_segment_handle || handles.size() == 1)
    return -1;
...
StrandHandle StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::Cut(StrandSegmentHandle target_segment_handle,
  ...
  new_strand.start_position = original_strand_last_segment.end_position =
      Strands::CubicInterpolation(p0, p1, p2, p3, t);
  new_strand.start_thickness = original_strand_last_segment.end_thickness =
      Strands::CubicInterpolation(t0, t1, t2, t3, t);
  ...
  new_strand.strand_segment_handles_.insert(
      new_strand.strand_segment_handles_.end(),
...
template <typename StrandGroupData, typename StrandData, typename StrandSegmentData>
void StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::CutLatter(
    const StrandSegmentHandle target_segment_handle) {
...
template <typename StrandGroupData, typename StrandData, typename StrandSegmentData>
void StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::RemoveStrand(
    const StrandHandle target_strand_handle) {
...
void StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::GetColorControlPoints(
    const StrandSegmentHandle strand_segment_handle, glm::vec4& p0, glm::vec4& p1, glm::vec4& p2, glm::vec4& p3) const {
  const auto& strand_segment = strand_segments_[strand_segment_handle];
  const auto& strand = strands_[strand_segment.strand_handle_];
  const auto& strand_segment_handles = strand.strand_segment_handles_;
...
void StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::FindStrandT(
  ...
  for (const auto& segment_handle : segment_handles) {
    ...
    if (length_sum + current_segment_length > target_length) {
      ...
      t = FindStrandSegmentT(result_strand_segment_handle, 0.f, target_length - length_sum, tolerance);
      return;
    }
  ...
  for (int strand_handle = 0; strand_handle < strands_.size(); strand_handle++) {
    ...
    for (uint32_t segment_index = 0; segment_index < strand.strand_segment_handles_.size(); segment_index++) {
      ...
      for (uint32_t subdivision_index = 0; subdivision_index < subdivision; subdivision_index++) {
        ...
        float start_root_distance =
            root_distance + Strands::CalculateLengthAdaptive(p0, p1, p2, p3, 0.0f, t_start, tolerance);
        float end_root_distance;
        if (subdivision_index == subdivision - 1 && segment_index == strand.strand_segment_handles_.size() - 1) {
          end_root_distance = GetStrandArcLength(strand_handle);
        } else {
          end_root_distance = root_distance + Strands::CalculateLengthAdaptive(p0, p1, p2, p3, 0.0f, t_end, tolerance);
        }
        segment_data_action(start_root_distance, end_root_distance, segment_handle, segment_index, t_end,
...
void StrandGroup<StrandGroupData, StrandData, StrandSegmentData>::Subdivide(
    StrandGroup<OSgd, OSd, OSsd>& target_strand_group, const std::function<float()>& target_segment_length,
    const std::function<void(StrandHandle src_handle, OSd& tgt_data)>& strand_data_action,
    const std::function<void(float start_root_distance, float end_root_distance, StrandSegmentHandle src_handle, uint32_t segment_index,
                             float segment_t, OSsd& tgt_data, uint32_t sub_segment_index)>& segment_data_action,
    float tolerance) const {
  target_strand_group.Clear();
  for (int strand_handle = 0; strand_handle < strands_.size(); strand_handle++) {
    ...
    float end_root_distance = remaining_length;
    uint32_t sub_segment_index = 0;
    for (uint32_t segment_index = 0; segment_index < strand.strand_segment_handles_.size(); segment_index++) {
      const auto& segment_handle = strand.strand_segment_handles_[segment_index];
      glm::vec3 p0, p1, p2, p3;
      ...
      while (true) {
        if (const float t_next = Strands::FindTAdaptive(p0, p1, p2, p3, t, remaining_length, tolerance);
          ...
          segment_data_action(previous_root_distance, end_root_distance, segment_handle, segment_index, t_next,
                              target_strand_group.RefStrandSegmentData(new_strand_segment_handle), sub_segment_index);
          sub_segment_index++;
          previous_root_distance = end_root_distance;
          t = t_next;
          remaining_length = target_segment_length();
          end_root_distance += remaining_length;
        } else {
    ...
    if (remaining_length != 0.f && !strand.strand_segment_handles_.empty()) {
      ...
      segment_data_action(previous_root_distance, GetStrandArcLength(strand_handle), segment_handle,
                          strand.strand_segment_handles_.size() - 1,
