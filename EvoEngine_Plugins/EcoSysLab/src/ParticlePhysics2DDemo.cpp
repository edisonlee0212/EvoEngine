...
bool ParticlePhysics2DDemo::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  ...
  ImGui::DragFloat("Particle Softness", &particle_physics_2d_.particle_physics_settings.particle_softness, 0.001f,
                   0.001f, 1.0f);
    ...
    if (ImGui::Begin(tag.c_str())) {
      ...
      particle_physics_2d_.OnInspect(
          ...
          [&](const ImVec2 origin, const float zoom_factor, ImDrawList* draw_list) {
            ...
            particle_physics_2d_.RenderEdges(origin, zoom_factor, draw_list, IM_COL32(0.0f, 0.0f, 128.0f, 128.0f),
                                             1.0f);
      ...
      if (last_frame_clicked) {
        if (mouse_down) {
          if (!add_attractor) {
            ...
            if (auto& attractor_points = profile_boundaries_.attractors.back().attractor_points;
