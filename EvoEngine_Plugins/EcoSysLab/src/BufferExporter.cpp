...
void MeshletObjExport::ExportObj(const std::filesystem::path& path,
  ...
  kinDS::VoronoiMesh mesh = ToVoronoiMesh(vertices, triangles, fracture_distance);
  ...
  options.gpu_attributes = BuildGpuAttributes(vertices, triangles, segments, uv_height_factor);
...
void MeshletObjExport::ExportObjCombined(const std::filesystem::path& path, const std::vector<MeshGroup>& groups,
  ...
  for (const auto& group : groups) {
    kinDS::VoronoiMesh part = ToVoronoiMesh(group.vertices, group.triangles, fracture_distance);
    kinDS::ObjExportGpuAttributes part_attrs =
        BuildGpuAttributes(group.vertices, group.triangles, segments, uv_height_factor);
