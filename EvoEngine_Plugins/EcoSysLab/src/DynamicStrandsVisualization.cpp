...
bool DynamicStrands::VisualizationParameters::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  ...
  if (ImGui::Checkbox("Particles", &render_particles))
    changed = true;
  if (render_particles) {
    if (ImGui::Combo("Particle mode", {"Default", "Segment color"}, particle_render_mode))
      changed = true;
    switch (particle_render_mode) {
      case 0: {
        if (ImGui::ColorEdit4("Particle color", &particle_color_main.x))
          changed = true;
        if (ImGui::DragFloat("Particle radius multiplier", &particle_radius_multiplier, 0.1f, 0.1f, 1000.f))
          changed = true;
        break;
      }
      case 1: {
  ...
  if (render_segments) {
    if (ImGui::Combo("Segment mode",
                     {"Default", "Segment color", "Shear strain", "Stretch strain", "Boundary distance"},
                     segment_render_mode))
      changed = true;
  ...
  if (render_connections) {
    if (ImGui::Combo("Connection mode", {"Default", "Bend strain", "Twist strain"}, connection_render_mode))
      changed = true;
    switch (connection_render_mode) {
      case 0: {
        if (ImGui::ColorEdit4("Connection color", &connection_color_main.x))
          changed = true;
  ...
  if (render_segment_pairs) {
    if (ImGui::Combo("Segment Pair mode", {"Default"}, segment_pair_render_mode))
      changed = true;
...
void DynamicStrands::Visualize(const std::shared_ptr<Camera>& target_camera,
  ...
  particle_push_constant.min_color = visualization_parameters.particle_color_main;
  particle_push_constant.camera_index = render_layer->GetCameraIndex(target_camera->GetHandle());
  particle_push_constant.multiplier = visualization_parameters.particle_radius_multiplier;
  ...
  segment_pair_push_constant.min_color = visualization_parameters.segment_pair_color_main;
  ...
  connection_push_constant.max_color = visualization_parameters.connection_color_max;
  connection_push_constant.camera_index = render_layer->GetCameraIndex(target_camera->GetHandle());
  connection_push_constant.multiplier = visualization_parameters.connection_radius_multiplier;
