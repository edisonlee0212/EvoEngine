...
bool InSameGroup(in UniformParticle p0, in UniformParticle p1) {
  return segments[p0.segment_handle].group_index == segments[p1.segment_handle].group_index;
}

bool RemainInSameGroup(in DelaunayTetrahedron tet) {
  UniformParticle p[4];
  [[unroll]]
  for (uint i = 0; i < 4; i++) {
...
vec3 ComputeTetTriangleNormal(in DelaunayTetrahedron tet, uint triangle_index) {
