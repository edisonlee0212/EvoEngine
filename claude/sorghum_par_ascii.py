...
def validate_daily_par(par_df):
    ...
    Expected range for clear June day at 33degN: 30-50 mol photons m-2 day-
    ...
    print(f"Expected range (clear sky): 30-50 mol photons m-2 day-")
    print()
    ...
    if 30 <= hourly_integral <= 50:
        ...
        print("[!] VALIDATION WARNING: Daily PAR slightly outside range but reasonable")
        status = "WARNING"
    else:
        print("[X] VALIDATION FAILED: Daily PAR outside expected range")
...
def create_visualization(par_df, output_dir):
    ...
    fig.suptitle(f'Sorghum Leaf Daily PAR - EvoEngine Ray Tracing (Target Build)\n' +
                 ...
                 f'Leaf: {LEAF_LENGTH_M*100:.0f} cm x {LEAF_WIDTH_M*100:.0f} cm, {LEAF_ANGLE_DEG}deg angle',
