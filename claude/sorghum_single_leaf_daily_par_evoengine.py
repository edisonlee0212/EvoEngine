...
def setup_leaf_geometry():
    ...
    if btf_path and hasattr(sorghum_framework, 'SetCBTFGroup'):
        ...
        print("  ✓ BTF enabled for realistic leaf optics")
    else:
...
def run_raytraced_illumination(sun_positions_df, sorghum_entity):
    ...
    print()
    print("✓ Ray-traced illumination complete")
...
def validate_daily_par(par_df):
    ...
    if 30 <= hourly_integral <= 50:
        print("✓ VALIDATION PASSED: Daily PAR within expected range")
        status = "PASS"
    elif 25 <= hourly_integral <= 55:
        print("⚠ VALIDATION WARNING: Daily PAR slightly outside range but reasonable")
        status = "WARNING"
    else:
        print("✗ VALIDATION FAILED: Daily PAR outside expected range")
