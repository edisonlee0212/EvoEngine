...
inline uint64_t endian_swap<uint64_t, uint64_t>(const uint64_t &v) noexcept {
  return (((v & 0x00000000000000ffLL) << 56) | ((v & 0x000000000000ff00LL) << 40) | ((v & 0x0000000000ff0000LL) << 24) |
...
size_t PlyFile::PlyFileImpl::read_property_binary(const size_t &stride, void *dest, size_t &destOffset,
...
void PlyFile::PlyFileImpl::parse_data(std::istream &is, bool firstPass) {
  ...
  if (isBinary) {
    ...
    read = [this, &listSize, &dummyCount](PropertyLookup &f, const PlyProperty &p, uint8_t *dest, size_t &destOffset,
                                          std::istream &_is) noexcept {
      if (!p.isList) {
