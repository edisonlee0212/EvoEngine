...
inline bool RectangleOverlapsLineSegment(const ImRect& rect, const ImVec2& p1, const ImVec2& p2) {
  ...
  if ((p1.x < flip_rect.Min.x && p2.x < flip_rect.Min.x) || (p1.x > flip_rect.Max.x && p2.x > flip_rect.Max.x) ||
      (p1.y < flip_rect.Min.y && p2.y < flip_rect.Min.y) || (p1.y > flip_rect.Max.y && p2.y > flip_rect.Max.y)) {
    return false;
  }
...
inline bool RectangleOverlapsBezier(const ImRect& rectangle, const CubicBezier& cubic_bezier) {
  ...
  for (int s = 0; s < cubic_bezier.NumSegments; ++s) {
    ImVec2 next = EvalCubicBezier(static_cast<float>((s + 1) * dt), cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2,
                                  cubic_bezier.P3);
...
inline bool RectangleOverlapsLink(const ImRect& rectangle, const ImVec2& start, const ImVec2& end,
  ...
  if (rectangle.Overlaps(lrect)) {
    ...
    const CubicBezier cubic_bezier = GetCubicBezier(start, end, start_type, GImNodes->Style.LinkLineSegmentsPerLength);
...
void DrawListAddNode(const int node_idx) {
  GImNodes->NodeIdxToSubmissionIdx.SetInt(static_cast<ImGuiID>(node_idx), GImNodes->NodeIdxSubmissionOrder.Size);
...
void BeginCanvasInteraction(ImNodesEditorContext& editor) {
  const bool any_ui_element_hovered = GImNodes->HoveredNodeIdx.HasValue() || GImNodes->HoveredLinkIdx.HasValue() ||
...
void TranslateSelectedNodes(ImNodesEditorContext& editor) {
  if (GImNodes->LeftMouseDragging) {
    ...
    const ImVec2 origin = SnapOriginToGrid(GImNodes->MousePos - GImNodes->CanvasOriginScreenSpace - editor.Panning +
...
bool ShouldLinkSnapToPin(const ImNodesEditorContext& editor, const ImPinData& start_pin, const int hovered_pin_idx,
  ...
  switch (editor.ClickInteraction.Type) {
      ...
      if (GImNodes->LeftMouseReleased) {
        ...
        if ((selected_idxs.Size > 0) && (selected_idxs.Size < depth_stack.Size)) {
          ...
          for (int i = 0; i < depth_stack.Size - selected_idxs.Size; ++i) {
            for (int node_idx = depth_stack[i]; selected_idxs.contains(node_idx); node_idx = depth_stack[i]) {
    ...
    case ImNodesClickInteractionType_LinkCreation: {
      ...
      const bool should_snap =
          GImNodes->HoveredPinIdx.HasValue() &&
          ShouldLinkSnapToPin(editor, start_pin, GImNodes->HoveredPinIdx.Value(), maybe_duplicate_link_idx);

      // If we created on snap and the hovered pin is empty or changed, then we need signal that
      // the link's state has changed.
      const bool snapping_pin_changed = editor.ClickInteraction.LinkCreation.EndPinIdx.HasValue() &&
      ...
      if (snapping_pin_changed && GImNodes->SnapLinkIdx.HasValue()) {
        BeginLinkDetach(editor, GImNodes->SnapLinkIdx.Value(), editor.ClickInteraction.LinkCreation.EndPinIdx.Value());
      }
      ...
      const ImVec2 end_pos =
          should_snap ? GetScreenSpacePinCoordinates(editor, editor.Pins.Pool[GImNodes->HoveredPinIdx.Value()])
                      : GImNodes->MousePos;
...
#endif
          cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2, cubic_bezier.P3, GImNodes->Style.Colors[ImNodesCol_Link],
          GImNodes->Style.LinkThickness / editor.ZoomScale, cubic_bezier.NumSegments);

      const bool link_creation_on_snap =
          GImNodes->HoveredPinIdx.HasValue() &&
          (editor.Pins.Pool[GImNodes->HoveredPinIdx.Value()].Flags & ImNodesAttributeFlags_EnableLinkCreationOnSnap);
...
inline ImVec2 GetNodeContentOrigin(const ImNodeData& node) {
  const ImVec2 title_bar_height = ImVec2(0.f, node.TitleBarContentRect.GetHeight() + 2.0f * node.LayoutStyle.Padding.y);
...
void DrawGrid(ImNodesEditorContext& editor, const ImVec2& canvas_size) {
  ...
  for (float x = fmodf(offset.x, GImNodes->Style.GridSpacing); x < canvas_size.x; x += GImNodes->Style.GridSpacing) {
  ...
  for (float y = fmodf(offset.y, GImNodes->Style.GridSpacing); y < canvas_size.y; y += GImNodes->Style.GridSpacing) {
...
void DrawPinShape(const ImVec2& pin_pos, const ImPinData& pin, const ImU32 pin_color) {
  ...
  switch (pin.Shape) {
    case ImNodesPinShape_Circle: {
      GImNodes->CanvasDrawList->AddCircle(pin_pos, GImNodes->Style.PinCircleRadius, pin_color, CIRCLE_NUM_SEGMENTS,
...
static void MiniMapDrawNode(ImNodesEditorContext& editor, const int node_idx) {
  ...
  GImNodes->CanvasDrawList->AddRect(node_rect.Min, node_rect.Max, mini_map_node_outline, mini_map_node_rounding, 0,
                                    1 / editor.ZoomScale);
}
...
static void MiniMapDrawLink(ImNodesEditorContext& editor, const int link_idx) {
  ...
  const CubicBezier cubic_bezier =
      GetCubicBezier(ScreenSpaceToMiniMapSpace(editor, start_pin.Pos), ScreenSpaceToMiniMapSpace(editor, end_pin.Pos),
...
#endif
      cubic_bezier.P0, cubic_bezier.P1, cubic_bezier.P2, cubic_bezier.P3, link_color,
      GImNodes->Style.LinkThickness * editor.MiniMapScaling / editor.ZoomScale, cubic_bezier.NumSegments);
}
...
static void MiniMapUpdate() {
  ...
  GImNodes->CanvasDrawList->AddRect(mini_map_rect.Min, mini_map_rect.Max,
                                    GImNodes->Style.Colors[ImNodesCol_MiniMapOutline], 0, 0, 1 / editor.ZoomScale);
...
void BeginNodeEditor() {
  ...
  ImGui::BeginGroup();
  {
    ...
    memcpy((void*)&GImNodes->NodeEditorImgCtx->IO, (void*)&GImNodes->OriginalImgCtx->IO,
           offsetof(ImGuiIO, PlatformSetImeDataFn) + sizeof(GImNodes->OriginalImgCtx->IO.PlatformSetImeDataFn));
    ...
    for (ImGuiInputEvent& e : GImNodes->NodeEditorImgCtx->InputEventsQueue) {
      if (e.Type == ImGuiInputEventType_MousePos) {
        e.MousePos.PosX = (e.MousePos.PosX - GImNodes->CanvasOriginalOrigin.x) / editor.ZoomScale;
        e.MousePos.PosY = (e.MousePos.PosY - GImNodes->CanvasOriginalOrigin.y) / editor.ZoomScale;
      }
    }
...

    {
      ...
      GImNodes->CanvasRectScreenSpace =
          ImRect(EditorSpaceToScreenSpace(ImVec2(0.f, 0.f)), EditorSpaceToScreenSpace(window_size));
  ...
  GImNodes->AltMouseDragging = (GImNodes->Io.EmulateThreeButtonMouse.Modifier != NULL && GImNodes->LeftMouseDragging &&
...
void EndNodeEditor() {
  ...
  GImNodes->OriginalImgCtx->WantTextInputNextFrame =
      ImMax(GImNodes->OriginalImgCtx->WantTextInputNextFrame, GImNodes->NodeEditorImgCtx->WantTextInputNextFrame);
...
float EditorContextGetZoom() {
  return EditorContextGet().ZoomScale;
}
...
bool IsLinkDropped(int* const started_at_id, const bool including_detached_links) {
  ...
  const bool link_dropped =
      ...
      (including_detached_links || editor.ClickInteraction.LinkCreation.Type != ImNodesLinkCreationType_FromDetach);
