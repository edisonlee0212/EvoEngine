...
void SerializeCamera(YAML::Emitter& out, const Camera& camera) {
  ...
  out << YAML::Key << "ray_integrator" << YAML::Value
      << Camera::GetRayIntegratorName(camera.camera_settings.ray_integrator);
...
void DeserializeCamera(const YAML::Node& in, Camera& camera) {
  ...
  if (in["ray_integrator"])
    camera.camera_settings.ray_integrator =
        Camera::ParseRayIntegrator(in["ray_integrator"].as<std::string>(), camera.camera_settings.ray_integrator);
...
void SerializePostProcessingStack(YAML::Emitter& out, const PostProcessingStack& stack) {
  ...
  out << YAML::Key << "enable_anti_aliasing" << YAML::Value << stack.enable_anti_aliasing;
  ...
  if (stack.anti_aliasing) {
    out << YAML::Key << "anti_aliasing" << YAML::Value << YAML::BeginMap;
    stack.anti_aliasing->Serialize(out);
    out << YAML::EndMap;
  }
...
void DeserializePostProcessingStack(const YAML::Node& in, PostProcessingStack& stack) {
  ...
  *stack.anti_aliasing = AntiAliasing{};
  ...
  if (in["enable_anti_aliasing"])
    stack.enable_anti_aliasing = in["enable_anti_aliasing"].as<bool>();
  ...
  if (in["anti_aliasing"]) {
    stack.anti_aliasing->Deserialize(in["anti_aliasing"]);
  }
  if (in["tone_mapping"]) {
    if (!stack.tone_mapping)
