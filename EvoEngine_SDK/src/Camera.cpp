...
void Camera::UpdateGBuffer() {
  ...
  }
  {
    ...
    image_info.mipLevels = 1;
    image_info.arrayLayers = 1;
    image_info.format = Platform::Constants::g_buffer_albedo;
    image_info.tiling = VK_IMAGE_TILING_OPTIMAL;
    image_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    image_info.usage = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT |
                       VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
    ...
    view_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
    view_info.format = Platform::Constants::g_buffer_albedo;
    view_info.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    view_info.subresourceRange.baseMipLevel = 0;
    view_info.subresourceRange.levelCount = 1;
    view_info.subresourceRange.baseArrayLayer = 0;
...
void Camera::TransitGBufferImageLayout(const VkCommandBuffer vk_command_buffer, VkImageLayout target_layout) const {
  ...
  g_buffer_albedo_->TransitImageLayout(vk_command_buffer, target_layout);
}
