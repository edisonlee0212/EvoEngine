...
void RecordProbeClassification(const VkCommandBuffer vk_command_buffer, const RenderGraphExecutionContext& context,
  ...
  if (!parameters.pipeline || !parameters.pipeline->Initialized() || !parameters.descriptor_set_layout ||
      !parameters.transient_resources || (!parameters.reset_classification && !parameters.classify_probes)) {
    return;
  }
...
RenderPassDescriptor DdgiProbeClassificationPass::CreateDescriptor() {
