...
void main() {
  ...
  if (visibility != 0) {
    outDstColorVisibility = max(vec4(texture(inColor, uv).rgb, visibility), vec4(0.0f));
  } else {
    outDstColorVisibility = vec4(0, 0, 0, 0);
  }
}