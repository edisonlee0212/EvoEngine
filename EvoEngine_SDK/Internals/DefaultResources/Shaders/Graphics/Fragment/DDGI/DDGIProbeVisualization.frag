...
layout(set = 1, binding = 18) uniform sampler2D EE_DDGI_VISIBILITY_ATLAS;

layout(push_constant) uniform EE_DDGI_PROBE_VISUALIZATION_CONSTANTS {
  uvec4 camera_probe_selected_mode;
...
void main() {
  const uint mode = camera_probe_selected_mode.w;
  ...
  const uint irradiance_columns = max(uint(EE_RENDER_INFO.ddgi_atlas_parameters.y), 1u);
  ...
  const vec3 atlas_irradiance = texture(EE_DDGI_IRRADIANCE_ATLAS, irradiance_uv).rgb;

  vec3 color = EE_DDGI_IRRADIANCE_DEBUG_COLOR(atlas_irradiance, radius_intensity_alpha_selected_scale.y);
  if (mode == 1u) {
