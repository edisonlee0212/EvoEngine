...
void main() {
  ...
  if (ndcDepth == 1.0) {
    if (!instance_selected && EE_INSTANCE_INDEX == 1) {
      ...
      for (int i = -3; i <= 3 && !foundNeighbor; ++i) {
        for (int j = -3; j <= 3; ++j) {
          int info_index_local =
              int(round(texture(inMaterial, fs_in.TexCoord + vec2(texOffset.x * float(i), texOffset.y * float(j))).w));
          if ((info_index_local & 1) == 1) {
            FragColor = mix(vec4(1.0, 0.75, 0.0, 1.0), vec4(skyColor, 1.0), 0.1);
            foundNeighbor = true;
            break;
          }
        }
  ...
  float roughness = EE_SAMPLE_TEXTURE_2D(materialProperties.roughness_map_index, materialTexCoord,
                                         vec4(materialProperties.roughness, 0, 0, 0))
                        .r;
  float metallic = EE_SAMPLE_TEXTURE_2D(materialProperties.metallic_map_index, materialTexCoord,
                                        vec4(materialProperties.metallic, 0, 0, 0))
                       .r;
  float emission = materialProperties.emission;
  float ao = EE_SAMPLE_TEXTURE_2D(materialProperties.ao_texture_index, materialTexCoord,
  ...
  vec3 debugColor = base * is0 + kMat * is1 + kInst * is2 + kInfo * is3;
  ...
  vec3 direct = EE_FUNC_CALCULATE_LIGHTS(receiveShadow, albedo.rgb, 1.0, depth, normal, viewDir, fragPos, metallic,
                                         roughness, F0);
  ...
  if (!instance_selected && EE_INSTANCE_INDEX == 1) {
    ...
    for (int i = -3; i <= 3 && !foundNeighbor; ++i) {
      for (int j = -3; j <= 3; ++j) {
        int info_index_local =
            int(round(texture(inMaterial, fs_in.TexCoord + vec2(texOffset.x * float(i), texOffset.y * float(j))).w));
        if ((info_index_local & 1) == 1) {
          outputColor = mix(vec4(1.0, 0.75, 0.0, 1.0), vec4(color, 1.0), 0.1);
          foundNeighbor = true;
          break;
        }
      }
    ...
    if (!foundNeighbor) {
      outputColor = mix(vec4(0.5, 0.5, 0.5, 1.0), vec4(color, 1.0), float(EE_LIGHT_SPLIT_INDEX) / 256.0);
    }
  } else {
