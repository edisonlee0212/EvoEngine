...
vec3 NishitaSkyIncidentLight(in vec3 camera_position, in vec3 rayDir){
	float earthRadius =
		atmosphere.earth_radius * 1000.0f;  // In the paper this is usually Rg or Re (radius ground, eart)
	...
	for (uint i = 0; i < numSamples; ++i) {
		...
		for (j = 0; j < numSamplesLight; ++j) {
			...
			samplePosition + (tCurrentLight + segmentLengthLight * 0.5f) * sun_direction;
...
void main()
{
	...
	vec3 color = NishitaSkyIncidentLight(vec3(0, 0, 0), ray_direction);
