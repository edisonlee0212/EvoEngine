...
void main() {
  ...
  const ivec3 probe_scroll_offset = ivec3(EE_RENDER_INFO.ddgi_probe_scroll_offset.xyz);
  ...
  const float selected = logical_probe_index == camera_probe_selected_mode.z ? 1.0f : 0.0f;
  ...
  const vec3 world_position =
      EE_DDGI_PROBE_WORLD_POSITION(vec3(probe_grid), EE_RENDER_INFO.ddgi_first_probe.xyz,
  ...
  vs_out.LogicalProbeIndex = logical_probe_index;
  vs_out.Selected = selected;

  const uint camera_index = camera_probe_selected_mode.x;
