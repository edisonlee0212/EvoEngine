...
void main()
{
	...
	vec3 N = normalize(transpose(inverse(mat3(boneTransform))) * inNormal);
	vec3 T = mat3(boneTransform) * inTangent;
	vs_out.Normal = N;
	vs_out.Tangent = T;
	transformHandedness = EE_TRANSFORM_HANDEDNESS(boneTransform);
