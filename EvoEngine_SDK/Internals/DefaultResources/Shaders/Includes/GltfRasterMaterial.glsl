...
GltfRasterMaterial EE_EVALUATE_GLTF_RASTER_SURFACE(
  ...
  if (material.pbr_model == EE_GLTF_PBR_MODEL_SPECULAR_GLOSSINESS) {
    ...
    diffuse *= EE_GLTF_SAMPLE_TEXTURE(material.pbr_diffuse_texture, tex_coord_0, tex_coord_1, vec4(1.0), tex_grad);
    vec4 specular_glossiness =
        EE_GLTF_SAMPLE_TEXTURE(material.pbr_specular_glossiness_texture, tex_coord_0, tex_coord_1, vec4(1.0),
                               tex_grad);
  ...
  {
    surface.base_color *=
        EE_GLTF_SAMPLE_TEXTURE(material.pbr_base_color_texture, tex_coord_0, tex_coord_1, vec4(1.0), tex_grad);
    vec4 metallic_roughness =
        EE_GLTF_SAMPLE_TEXTURE(material.pbr_metallic_roughness_texture, tex_coord_0, tex_coord_1, vec4(1.0),
                               tex_grad);
  ...
  if (EE_GLTF_HAS_TEXTURE(material.occlusion_texture)) {
    float occlusion =
        EE_GLTF_SAMPLE_TEXTURE(material.occlusion_texture, tex_coord_0, tex_coord_1, vec4(1.0), tex_grad).r;
  ...
  surface.emissive *=
      EE_GLTF_SAMPLE_TEXTURE(material.emissive_texture, tex_coord_0, tex_coord_1, vec4(1.0), tex_grad).rgb;
...
vec3 EE_EVALUATE_GLTF_RASTER_NORMAL(uint material_index, vec2 tex_coord_0, vec2 tex_coord_1, vec3 normal,
  ...
  vec3 normal_vector =
      EE_GLTF_SAMPLE_TEXTURE(material.normal_texture, tex_coord_0, tex_coord_1, vec4(0.5, 0.5, 1.0, 1.0),
                             tex_grad).xyz;
...
float EE_GLTF_RASTER_OPACITY_LOD0(uint material_index, vec2 tex_coord_0, vec2 tex_coord_1, float vertex_alpha) {
  ...
  if (material.pbr_model == EE_GLTF_PBR_MODEL_SPECULAR_GLOSSINESS) {
    ...
    base_color_alpha *=
        EE_GLTF_SAMPLE_TEXTURE_LOD0(material.pbr_diffuse_texture, tex_coord_0, tex_coord_1, vec4(1.0)).a;
  } else
#endif
  {
    base_color_alpha = material.pbr_base_color_factor.a;
    base_color_alpha *=
        EE_GLTF_SAMPLE_TEXTURE_LOD0(material.pbr_base_color_texture, tex_coord_0, tex_coord_1, vec4(1.0)).a;
  }
...
vec3 EE_GLTF_RASTER_SHADOW_TRANSMISSION_LOD0(uint material_index, vec2 tex_coord_0, vec2 tex_coord_1,
  ...
  const vec4 metallic_roughness =
      EE_GLTF_SAMPLE_TEXTURE_LOD0(material.pbr_metallic_roughness_texture, tex_coord_0, tex_coord_1, vec4(1.0));
