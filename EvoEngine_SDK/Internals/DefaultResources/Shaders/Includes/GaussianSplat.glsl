...
const uint EE_GAUSSIAN_SPLAT_SORTED_INDICES_FLAG = 1u;
const uint EE_GAUSSIAN_SPLAT_INDIRECT_DRAW_FLAG = 2u;
...
#if EE_GAUSSIAN_SPLAT_ENABLE_DRAW_COMMAND
...
layout(set = 1, binding = 3) readonly buffer EE_GAUSSIAN_SPLAT_INDIRECT_DRAW_BLOCK {
...
const vec2 EE_GAUSSIAN_SPLAT_TRIANGLE_CORNERS[6] = vec2[](
    vec2(-1.0f, -1.0f), vec2(1.0f, -1.0f), vec2(-1.0f, 1.0f),
    vec2(-1.0f, 1.0f), vec2(1.0f, -1.0f), vec2(1.0f, 1.0f));
const vec2 EE_GAUSSIAN_SPLAT_QUAD_CORNERS[4] =
...
uint EE_GAUSSIAN_SPLAT_DRAW_COUNT() {
#if EE_GAUSSIAN_SPLAT_ENABLE_DRAW_COMMAND
  if ((camera_instance_count_flags.w & EE_GAUSSIAN_SPLAT_INDIRECT_DRAW_FLAG) != 0u) {
    return EE_GAUSSIAN_SPLAT_DRAW_COMMAND.instanceCount;
  }
#endif
  return camera_instance_count_flags.z;
}
...
void EE_GAUSSIAN_SPLAT_BUILD_VERTEX(const uint splat_order_index, const vec2 corner, out vec4 clip_position,
                                    out vec3 color, out vec2 local_coord, out float opacity,
                                    out float cutoff_radius_squared) {
  const bool sorted = (camera_instance_count_flags.w & EE_GAUSSIAN_SPLAT_SORTED_INDICES_FLAG) != 0u;
  ...
  const float scaled_opacity = clamp(decoded_opacity * opacity_extent_min_max.x, 0.0f, 1.0f);
  color = splat.color_rest_offset.rgb * EE_GAUSSIAN_SPLAT_SH_C0 + vec3(0.5f);
  ...
  if (center_clip.w <= 0.0f || scaled_opacity <= 1.0f / 255.0f) {
    clip_position = vec4(0.0f, 0.0f, 2.0f, 1.0f);
    return;
  }
  mat2 axes;
