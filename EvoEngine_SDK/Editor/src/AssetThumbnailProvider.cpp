...
void EnsureDefaultPreviewHandlers() {
  AssetPreviewRegistry::RegisterAssetPreviewHandler<Texture2D>(
      [](const std::shared_ptr<Texture2D>& texture, const OffscreenPreviewSettings&) -> std::shared_ptr<Texture2D> {
        if (!texture)
          return std::shared_ptr<Texture2D>{};
        std::shared_ptr<Texture2D> ret_val = AssetManager::CreateTemporaryAsset<Texture2D>();
        const glm::vec2 resolution = texture->GetResolution();
        const float max_dim = glm::max(resolution.x, resolution.y);
        const glm::vec2 new_resolution = resolution * glm::min(1.f, 512.f / max_dim);
        const auto& local_data = texture->GetLocalData();
        if (local_data.empty()) {
          return {};
        }
        auto copy_data = local_data;
        Texture2D::Resize(local_data, resolution, copy_data, new_resolution);
        ret_val->SetRgbaChannelData(copy_data, new_resolution, false);
        return ret_val;
      },
      {}, "Texture2D");
  AssetPreviewRegistry::RegisterAssetPreviewHandler<Material>(
      ...
      {}, "Material");
  AssetPreviewRegistry::RegisterAssetPreviewHandler<Mesh>(
      ...
      {}, "Mesh");
  AssetPreviewRegistry::RegisterAssetPreviewHandler<Prefab>(
      [](const std::shared_ptr<Prefab>& prefab, const OffscreenPreviewSettings&) {
        return prefab ? EditorLayer::FindIcon("Prefab") : nullptr;
      },
      {}, "Prefab");
  AssetPreviewRegistry::RegisterAssetPreviewHandler<Scene>(
      [](const std::shared_ptr<Scene>& scene, const OffscreenPreviewSettings&) {
        return scene ? EditorLayer::FindIcon("Scene") : nullptr;
      },
      {}, "Scene");
}
}  // namespace
...
bool AssetThumbnailProvider::SupportsGeneratedThumbnail(const std::string& asset_type_name) {
  ...
  return AssetPreviewRegistry::HasAssetPreviewHandler(asset_type_name) ||
         (Serialization::HasAssetType(asset_type_name) && AssetPreviewRegistry::HasAssetPreviewHandler<IAsset>());
}
...
std::shared_ptr<Texture2D> AssetThumbnailProvider::GenerateThumbnail(const std::shared_ptr<IAsset>& asset,
  ...
  const auto thumbnail = AssetPreviewRegistry::GenerateAssetThumbnail(asset, settings);
