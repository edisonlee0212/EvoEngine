...
void EntitySelectionHighlightPass::Record(VkCommandBuffer vk_command_buffer, const RenderGraphExecutionContext& context,
                                          const Parameters& parameters) {
  if (!parameters.presentation.active || !parameters.camera || !parameters.camera->GetRenderTexture() ||
      !parameters.pipeline || !parameters.pipeline->Initialized() || !parameters.camera->GetGBufferDescriptorSet()) {
    return;
  }
