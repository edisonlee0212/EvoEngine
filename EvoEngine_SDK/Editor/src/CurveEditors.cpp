...
bool evo_engine::editor_widgets::Draw(Curve2D& curve, const std::string& label, const ImVec2& editor_size,
                                      unsigned flags) {
  enum class StorageValues : ImGuiID { FromX = 100, FromY, Width, Height, IsPanning, PointStartX, PointStartY };
  int changed_idx = -1;
  bool changed = false;

  bool no_tangent = !curve.IsTangent();
  ...
  float from_x = window->StateStorage.GetFloat(static_cast<ImGuiID>(StorageValues::FromX), curve.GetMin().x);
  float from_y = window->StateStorage.GetFloat(static_cast<ImGuiID>(StorageValues::FromY), curve.GetMin().y);
  float width =
      window->StateStorage.GetFloat(static_cast<ImGuiID>(StorageValues::Width), curve.GetMax().x - curve.GetMin().x);
  float height =
      window->StateStorage.GetFloat(static_cast<ImGuiID>(StorageValues::Height), curve.GetMax().y - curve.GetMin().y);
  ...
  for (int point_idx = points_count - 2; point_idx >= 0; --point_idx) {
    ...
    if (!no_tangent) {
      ...
      if (point_idx < points_count - 2 && handle_point(p, 1)) {
        points[3] = ImClamp(p, ImVec2(p_prev.x + tangent_last.x - tangent.x + 0.001f, curve.GetMin().y),
                            ImVec2(points[6].x + points[5].x - points[4].x - 0.001f, curve.GetMax().y));
        changed_idx = point_idx + 1;
      }
    } else {
  ...
  if (!((unsigned)flags & (unsigned)CurveEditorFlags::DisableStartEndY)) {
    if (no_tangent) {
      if (ImGui::SliderFloat("Begin Y", &values.front().y, curve.GetMin().y, curve.GetMax().y)) {
        changed = true;
      }
      if (ImGui::SliderFloat("End Y", &values.back().y, curve.GetMin().y, curve.GetMax().y)) {
        changed = true;
      }
    } else {
      if (ImGui::SliderFloat("Begin Y", &values[1].y, curve.GetMin().y, curve.GetMax().y)) {
        changed = true;
      }
      if (ImGui::SliderFloat("End Y", &values[values.size() - 2].y, curve.GetMin().y, curve.GetMax().y)) {
        changed = true;
      }
    }
  }
  if ((unsigned)flags & (unsigned)CurveEditorFlags::ShowDebug) {
    ...
    ImGui::Text("Y: %.3f", curve.GetValue(test));
  }
  return changed_idx != -1 || changed;
}

void evo_engine::editor_widgets::Draw(BezierSpline& spline) {
  int size = spline.curves.size();
  if (ImGui::DragInt("Size of curves", &size, 0, 10)) {
    size = glm::clamp(size, 0, 10);
    spline.curves.resize(size);
  }
