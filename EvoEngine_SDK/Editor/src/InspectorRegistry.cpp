...
InspectorRegistry& InspectorRegistry::GetInstance() {
  static std::unordered_map<Application*, std::unique_ptr<InspectorRegistry>> registries;
  auto* application = &ApplicationContext::Get();
  auto& registry = registries[application];
  if (!registry) {
    registry = std::make_unique<InspectorRegistry>();
    const auto cleanup = PackageManager::RegisterTypeCleanupCallback([application](const std::string& owner) {
      const auto it = registries.find(application);
      if (it != registries.end())
        it->second->UnregisterOwner(owner);
    });
...
bool InspectorRegistry::InspectValue(InspectorContext& context, const std::type_info& type, void* target) const {
  if (const auto* record = FindRecord(type)) {
    const auto handler = record->handler;
...
bool InspectorRegistry::InspectBatch(InspectorContext& context,
  ...
  if (search == batch_handlers_.end())
    return false;
  const auto handler = search->second.handler;
  return handler(context, targets);
}
