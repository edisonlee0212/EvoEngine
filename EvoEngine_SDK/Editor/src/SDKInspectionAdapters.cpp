...
bool InspectProceduralNoiseGraph(pn::IProceduralNoise& noise, const std::string& window_title,
  ...
  if (ImGui::Begin(window_title.c_str())) {
    ...
    NodeGraphEditor::Draw(
        graph, dynamic_cast<IAsset&>(noise).GetSelf(), id, editor_layer,
...
void DrawProceduralPreviewTexture(const std::shared_ptr<Texture2D>& texture) {
  ...
  const auto texture_id =
      EditorTextureRegistry::GetTextureId(texture_storage.image, texture_storage.image_view, texture_storage.sampler);
  if (!texture_id) {
    return;
  }
...
bool InspectCubemap(InspectorContext& context, Cubemap& cubemap) {
  ...
  if (const auto& storage = cubemap.RefStorage(); !storage.face_views.empty()) {
    static float debug_scale = 0.25f;
    ImGui::DragFloat("Scale", &debug_scale, 0.01f, 0.1f, 1.0f);
    debug_scale = glm::clamp(debug_scale, 0.1f, 1.0f);
    for (int i = 0; i < 6; i++) {
      ImGui::Image(
          EditorTextureRegistry::GetTextureId(storage.image, storage.face_views[i], storage.sampler),
...
bool InspectTexture2D(InspectorContext& context, Texture2D& texture) {
  ...
  if (const auto texture_id = EditorTextureRegistry::GetTextureId(texture_storage.image, texture_storage.image_view,
      ...
      texture_id && texture_storage.image) {
    static float debug_scale = 0.25f;
    ImGui::DragFloat("Scale", &debug_scale, 0.01f, 0.1f, 10.0f);
    debug_scale = glm::clamp(debug_scale, 0.1f, 10.0f);
    ImGui::Image(texture_id,
...
void evo_engine::DrawCameraDebugViews(const Camera& camera, const float debug_scale) {
  ...
  if (ImGui::TreeNodeEx("Base color / AO", ImGuiTreeNodeFlags_DefaultOpen)) {
    ImGui::Image(EditorTextureRegistry::GetTextureId(camera.GetGBufferBaseColorAoResources()), image_size, ImVec2(0, 1),
                 ImVec2(1, 0));
    ImGui::TreePop();
  }
  if (ImGui::TreeNodeEx("Normal / Roughness", ImGuiTreeNodeFlags_DefaultOpen)) {
    ImGui::Image(EditorTextureRegistry::GetTextureId(camera.GetGBufferNormalRoughnessResources()), image_size,
                 ImVec2(0, 1), ImVec2(1, 0));
    ImGui::TreePop();
  }
  if (ImGui::TreeNode("PBR / Flags")) {
    ImGui::Image(EditorTextureRegistry::GetTextureId(camera.GetGBufferPbrFlagsResources()), image_size, ImVec2(0, 1),
                 ImVec2(1, 0));
    ImGui::TreePop();
  }
  if (ImGui::TreeNode("Emissive")) {
    ImGui::Image(EditorTextureRegistry::GetTextureId(camera.GetGBufferEmissiveResources()), image_size, ImVec2(0, 1),
                 ImVec2(1, 0));
    ImGui::TreePop();
  }
  ...
  if (ImGui::TreeNode("Depth")) {
    ImGui::Image(EditorTextureRegistry::GetDepthTextureId(*camera.GetRenderTexture()), image_size, ImVec2(0, 1),
                 ImVec2(1, 0));
    ImGui::TreePop();
  }
}
