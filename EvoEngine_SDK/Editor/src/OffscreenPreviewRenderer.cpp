...
auto& PreviewContexts() {
  static std::unordered_map<Application*, std::unique_ptr<PreviewContext>> contexts;
  return contexts;
}
...
PreviewContext* GetPreviewContext() {
  ...
  auto [it, inserted] = PreviewContexts().try_emplace(application);
  if (inserted)
    static_cast<void>(application->RegisterCleanupFunction([application] {
      PreviewContexts().erase(application);
    }));
  auto& preview_context = it->second;
...
void OffscreenPreviewRenderer::Reset() {
  const auto it = PreviewContexts().find(ApplicationContext::TryGet());
  if (it != PreviewContexts().end())
    it->second.reset();
}
...
std::shared_ptr<Texture2D> OffscreenPreviewRenderer::CopyColorTexture(const std::shared_ptr<Camera>& camera,
  ...
  Platform::ImmediateSubmit([&](const VkCommandBuffer command_buffer) {
    ...
    const auto copy_image = reinterpret_cast<PFN_vkCmdCopyImage>(Platform::GetVulkanInstanceFunction("vkCmdCopyImage"));
    copy_image(command_buffer, source->GetVkImage(), VK_IMAGE_LAYOUT_GENERAL, target->GetVkImage(),
