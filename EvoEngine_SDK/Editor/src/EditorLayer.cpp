...
void DrawFittedImage(ImDrawList* draw_list, const std::shared_ptr<Texture2D>& icon, const ImRect& rect,
  ...
  draw_list->AddImage(EditorTextureRegistry::GetTextureId(*icon), min, ImVec2(min.x + size.x, min.y + size.y),
...
void EditorLayer::OnRuntimeStart() {
  ClearConsoleOnRuntimeStart();
}

void EditorLayer::RegisterTypes(Application&) {
  RegisterSdkInspectionAdapters();
  if (!EditorPackageCoordinator::Initialize())
...
void EditorLayer::OnCreate() {
  ...
  previous_auto_render_main_camera_ = window_layer->GetAutoRenderMainCamera();
  window_layer->SetAutoRenderMainCamera(false);
  RegisterProjectCallbacks();
  InitializeGizmoPipelines();
  ...
  entity_index_read_buffer_create_info.flags =
  ...
  mapped_entity_index_data_ = entity_index_read_buffer_->GetVmaAllocationInfo().pMappedData;
...
void EditorLayer::OnDestroy() {
  ...
  if (const auto render = GetApplication().GetLayer<RenderLayer>())
    render->SetAuxiliaryCameraProvider({});
  GetApplication().SetRuntimeOperationBusyPredicate({});
  ProjectManager::SetHostCallbacks({});
  gizmos.reset();
  gizmos_normal_colored.reset();
  gizmos_vertex_colored.reset();
  gizmos_instanced_colored.reset();
  gizmos_strands.reset();
  gizmos_strands_normal_colored.reset();
  gizmos_strands_vertex_colored.reset();

  scene_loading_popup_visible_ = false;
  if (const auto window = ApplicationContext::Get().GetLayer<WindowLayer>())
    window->SetAutoRenderMainCamera(previous_auto_render_main_camera_);
  ClearAssetInspectors();
...
void EditorLayer::DrawAssetInspectorWindows() {
  ...
  for (size_t i = 0; i < inspecting_assets_.size(); ++i) {
    ...
    if (ImGui::Begin(window_title.c_str(), &open)) {
      DrawAssetInspectorContent(editor_layer, asset);
    }
    ImGui::End();
...
void EditorLayer::DrawProjectLoadingPopup() {
  ...
  if (scene_loading_popup_visible_ && scene_ready) {
  ...
  } else if (scene_loading_popup_visible_ || pending_scene_load) {
  ...
  if (ImGui::BeginPopupModal("Loading Scene...", nullptr, modal_flags)) {
    ...
    if (!scene_loading_popup_visible_ && !pending_scene_load) {
...
void EditorLayer::RegisterEditorPanels() {
  ...
  build_manager_panel_ = std::make_shared<BuildManagerPanel>();
  GetApplication().SetRuntimeOperationBusyPredicate([panel = std::weak_ptr(build_manager_panel_)] {
    const auto locked = panel.lock();
    return locked && locked->ExportActive();
  });
  editor_panel_manager_.RegisterPanel(EditorPanelCategory::View, "build_manager", "Build Manager",
                                      show_build_manager_window, build_manager_panel_);
  ...
  register_panel("resources", "Resources", show_resources_, [](const std::shared_ptr<EditorLayer>& editor_layer) {
    DrawResources(editor_layer);
  });
  project_content_browser_panel_ = std::make_shared<ProjectContentBrowserPanel>();
  editor_panel_manager_.RegisterPanel(EditorPanelCategory::View, "project", "Project", show_project_window,
...
void EditorLayer::DrawRuntimePackageManagerWindow() {
  ...
  if (ImGui::Begin("Runtime Package Manager", &package_manager_open)) {
    ...
    const auto draw_loaded_package_inspection = [&](const LoadedPackageInfo& package) {
      ...
      if (!package.ready)
        ImGui::TextWrapped("Activation failed. Release remaining package objects, then unload or reload it.");
...
void EditorLayer::DrawMainMenuItems(const bool title_bar_style) {
  ...
  if (begin_menu("Window")) {
    ...
    if (ImGui::BeginMenu("Content")) {
      panel_menu_item("Project", show_project_window);
      panel_menu_item("Resources", show_resources_);
      ImGui::EndMenu();
    }
    panel_menu_item("Console", show_console_window);
    panel_menu_item("Runtime Packages", show_package_manager_window);
    panel_menu_item("Build Manager", show_build_manager_window);
    ImGui::Separator();
...
void EditorLayer::RequestEditorLayout(const EditorLayoutSettings& settings) {
  ...
  apply_visibility(settings.panels.project, show_project_window);
  apply_visibility(settings.panels.resources, show_resources_);
    ...
    if (scene) {
      ...
      if (ImGui::BeginChild("SceneCameraRenderer", ImVec2(0, 0), false)) {
        ...
        if (scene_camera && scene_camera->Rendered()) {
          ...
          ImGui::Image(EditorTextureRegistry::GetColorTextureId(*scene_camera->GetRenderTexture()),
...
bool EditorLayer::UnregisterEditorCamera(const Handle& camera_handle) {
  if (camera_handle == scene_camera_handle_)
    return false;
  return editor_cameras_.erase(camera_handle) != 0;
}
