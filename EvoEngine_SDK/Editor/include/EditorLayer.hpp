...
class EVOENGINE_EDITOR_API ProjectContentBrowserPanel;
 ...
 */
struct EVOENGINE_EDITOR_API GizmoSettings {
 ...
 */
class BuildManagerPanel;

class EVOENGINE_EDITOR_API EditorLayer : public ILayer {
  bool scene_loading_popup_visible_ = false;
  std::string close_project_error_;
  bool previous_auto_render_main_camera_ = true;

 public:
  void RegisterProjectCallbacks();
  ...
  static void ClearInspectorPreviewState();
  ...
  // The primary scene camera remains registered for the lifetime of the editor layer.
  bool UnregisterEditorCamera(const Handle& camera_handle);
  ...
  bool show_package_manager_window = false;
  bool show_build_manager_window = false;   /**< Indicates whether the runtime package manager window is visible. */
...
#pragma endregion
 ...
 private:
  ...
  void InitializeGizmoPipelines();
  void RegisterCameraProvider();
  void InitializeSelectionRendering();
  void RegisterSelectionPass();
  void ClearSelectionRendering();
 ...
 private:
  ...
  static void DrawResources(const std::shared_ptr<EditorLayer>& editor_layer);
   ...
   */
  bool OnInputEvent(const Input::InputEvent& input_event) override;
  ...
  std::shared_ptr<BuildManagerPanel> build_manager_panel_;
