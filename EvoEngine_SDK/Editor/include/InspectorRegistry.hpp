...
class EVOENGINE_EDITOR_API InspectorRegistry final {
 public:
  ...
  template <typename T>
  bool InspectValue(InspectorContext& context, T& target) const {
    if constexpr (std::is_polymorphic_v<T>)
      return InspectValue(context, typeid(target), dynamic_cast<void*>(&target));
    else
      return InspectValue(context, typeid(target), &target);
  }
  bool InspectValue(InspectorContext& context, const std::type_info& type, void* target) const;
