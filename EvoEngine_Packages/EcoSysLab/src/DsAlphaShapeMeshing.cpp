...
void eco_sys_lab_package::DsAlphaShapeMeshing::BuildRenderComputePipelines() {
...
void eco_sys_lab_package::DsAlphaShapeMeshing::Download() {
...
void DsAlphaShapeMeshing::Visualize(const std::shared_ptr<Camera>& target_camera,
  ...
  if (!uniform_particle_render_pipeline) {
    ...
    uniform_particle_render_pipeline->descriptor_set_layouts.emplace_back(
    ...
    if (render_settings.meshing_visualization_parameters.render_uniform_particles) {
      ...
      target_camera->GetRenderTexture()->Render(
          vk_command_buffer, VK_ATTACHMENT_LOAD_OP_LOAD, VK_ATTACHMENT_STORE_OP_STORE, [&] {
            ...
            uniform_particle_render_pipeline->DrawMeshTasks(vk_command_buffer, count, 1, 1);
          });
    }
  });
}
...
void eco_sys_lab_package::DsAlphaShapeMeshing::RegisterBranchesWireframeRenderInstance(
    Handle& rendering_instance_handle, std::shared_ptr<Scene> scene, Entity& owner) {
  const auto render_layer = ApplicationContext::Get().GetLayer<RenderLayer>();
  if (!render_layer) {
    EVOENGINE_LOG("Failed to render! RenderLayer not present!")
    return;
