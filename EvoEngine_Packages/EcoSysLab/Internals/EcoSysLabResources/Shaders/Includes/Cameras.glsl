...
struct Camera {
  ...
  uint camera_block_reserved4;
  uint ray_debug_view;
  uint raster_lighting_flags;
  uint ray_output_flags;
  uint camera_block_reserved0;
  uint camera_block_reserved1;
  uint camera_block_reserved2;
  vec4 shadow_split_distances;
};

const uint EE_CAMERA_RAY_OUTPUT_ALBEDO = 1u << 0u;
const uint EE_CAMERA_RAY_OUTPUT_NORMAL = 1u << 1u;
const uint EE_CAMERA_RAY_OUTPUT_RAY_COUNT = 1u << 2u;
const uint EE_CAMERA_RAY_OUTPUT_PATH_LENGTH = 1u << 3u;
const uint EE_CAMERA_RAY_OUTPUT_TIME = 1u << 4u;
const uint EE_CAMERA_RAY_OUTPUT_DEBUG = 1u << 5u;
bool EE_CAMERA_RAY_OUTPUT_ENABLED(const Camera camera, const uint output_flag) {
