        ...
        if (selected_node_handle != -1 && selected_node_handle < skeleton.RefRawNodes().size()) {
          ...
          DrawProfileCanvas(
              ...
              [&](const ImVec2 origin, const float zoom_factor, ImDrawList* draw_list) {
                DrawProfileEdges(node.data.profile, origin, zoom_factor, draw_list,
                                 IM_COL32(0.0f, 0.0f, 128.0f, 128.0f), 1.0f);
                DrawProfileBoundary(node.data.profile, origin, zoom_factor, draw_list,
                ...
                if (node.GetParentHandle() != -1) {
                  if (const auto& parent_node = skeleton.RefNode(node.GetParentHandle());
                    ...
                    for (const auto& parent_boundary : parent_node.data.profile_constraints.boundaries) {
                      DrawProfileBoundary(parent_boundary, origin, zoom_factor, draw_list,
                                          IM_COL32(128.0f, 0.0f, 0, 128.0f), 4.0f);
                    }
                    for (const auto& parent_attractor : parent_node.data.profile_constraints.attractors) {
                      DrawProfileAttractor(parent_attractor, origin, zoom_factor, draw_list,
                                           IM_COL32(0.0f, 128.0f, 0, 128.0f), 4.0f);
                    }
                  }
                }
                for (const auto& boundary : node.data.profile_constraints.boundaries) {
                  DrawProfileBoundary(boundary, origin, zoom_factor, draw_list, IM_COL32(255.0f, 0.0f, 0, 255.0f),
                                      2.0f);
                }

                for (const auto& attractor : node.data.profile_constraints.attractors) {
                  DrawProfileAttractor(attractor, origin, zoom_factor, draw_list, IM_COL32(0.0f, 255.0f, 0, 255.0f),
                                       2.0f);
                }
              },
              show_grid);
