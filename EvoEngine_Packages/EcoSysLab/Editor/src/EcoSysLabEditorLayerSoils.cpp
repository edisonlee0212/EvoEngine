...
void EcoSysLabEditorLayer::SoilVisualization() {
  const auto runtime = ApplicationContext::Get().GetLayer<EcoSysLabLayer>();
  if (!runtime)
    return;

  std::shared_ptr<Soil> soil;
  if (const auto soil_candidate = runtime->FindSoil(); !soil_candidate.expired())
...
void EcoSysLabEditorLayer::SoilVisualizationVector(const VoxelSoilModel& soil_model) {
  ...
  editor_layer->DrawGizmoMeshInstancedColored(Resources::GetInstance().GetPrimitives().cylinder, vector_matrices_,
                                              glm::mat4(1.0f), 1.0f, gizmo_settings);
}

bool EcoSysLabEditorLayer::SoilVisualizationSettings::DrawGui(const std::shared_ptr<EditorLayer>& editor_layer) {
  const auto runtime = ApplicationContext::Get().GetLayer<EcoSysLabLayer>();
  if (!runtime)
    return false;
  bool changed = false;
  ...
  if (scalar_enable) {
    ...
    if (scalar_enable && ImGui::TreeNodeEx("Scalar", ImGuiTreeNodeFlags_DefaultOpen)) {
      ...
      if (ImGui::TreeNodeEx("Layer colors", ImGuiTreeNodeFlags_DefaultOpen)) {
        for (int i = 0; i < 10; i++) {
          ImGui::ColorEdit4(("Layer " + std::to_string(i)).c_str(), &runtime->soil_layer_colors[i].x);
        }
        ImGui::TreePop();
      }
...
void EcoSysLabEditorLayer::SoilVisualizationScalar(const VoxelSoilModel& soil_model) {
  const auto runtime = ApplicationContext::Get().GetLayer<EcoSysLabLayer>();
  if (!runtime)
    return;
  const auto num_voxels = soil_model.m_resolution.x * soil_model.m_resolution.y * soil_model.m_resolution.z;
      ...
      case SoilProperty::SoilLayer: {
        Jobs::RunParallelFor(num_voxels, [&](size_t i) {
          ...
          if (layerIndex == 0)
            ...
            particle_infos[i].instance_color = runtime->soil_layer_colors[layerIndex - 1];
          }
        });
      } break;
  ...
  editor_layer->DrawGizmoMeshInstancedColored(Resources::GetInstance().GetPrimitives().cube, scalar_matrices_,
                                              glm::mat4(1.0f), 1.0f, gizmo_settings);
}
