...
void SpatialPlantDistributionInspector::DrawCanvas(
...
bool SpatialPlantDistributionInspector::Inspect(InspectorContext& context, SpatialPlantDistributionSimulator& target) {
  bool changed = false;
  if (ImGui::TreeNode("Global Parameters")) {
    ImGui::DragFloat("Weighting Factor", &target.m_distribution.m_spatialPlantGlobalParameters.m_p, 0.01f, 0.0f, 1.0f);
    ImGui::DragFloat("Delta", &target.m_distribution.m_spatialPlantGlobalParameters.m_p, 1.f, 1.0f, 3.0f);
    ImGui::DragFloat("Simulation Rate", &target.m_distribution.m_spatialPlantGlobalParameters.m_simulationRate, 0.01f,
                     0.0f, 10.0f);
    ImGui::DragFloat("Spawn Protection Factor",
                     &target.m_distribution.m_spatialPlantGlobalParameters.m_spawnProtectionFactor, 0.01f, 0.0f, 1.0f);
    ImGui::DragFloat("Max Radius", &target.m_distribution.m_spatialPlantGlobalParameters.m_maxRadius, 1.f, 10.0f,
                     10000.0f);
    ImGui::Checkbox("Force remove all overlap",
                    &target.m_distribution.m_spatialPlantGlobalParameters.m_forceRemoveOverlap);
    ImGui::DragFloat("Dynamic balance factor",
                     &target.m_distribution.m_spatialPlantGlobalParameters.m_dynamicBalanceFactor, 0.1f, 0.0f, 3.0f);
    ImGui::TreePop();
  }

  if (ImGui::TreeNode("Parameters")) {
    if (ImGui::Button("Push")) {
      target.m_distribution.m_spatialPlantParameters.emplace_back();
      target.m_distribution.m_spatialPlantParameters.back().m_color = glm::vec4(glm::abs(glm::sphericalRand(1.f)), 1.f);
      target.m_treeDescriptors.resize(target.m_distribution.m_spatialPlantParameters.size());
    }
    for (int parameterIndex = 0; parameterIndex < target.m_distribution.m_spatialPlantParameters.size();
         parameterIndex++) {
      if (ImGui::TreeNode(("No. " + std::to_string(parameterIndex)).c_str())) {
        auto& parameter = target.m_distribution.m_spatialPlantParameters.at(parameterIndex);
        ...
        context.editor_layer->DragAndDropButton<TreeDescriptor>(target.m_treeDescriptors.at(parameterIndex),
                                                                "Tree Descriptor", true);
        ImGui::ColorEdit4("Color", &parameter.m_color.x);
        if (ImGui::Button((std::string("Clear No. ") + std::to_string(parameterIndex)).c_str())) {
          for (const auto& plant : target.m_distribution.m_plants) {
            if (!plant.m_recycled && plant.m_parameterHandle == parameterIndex)
              target.m_distribution.RecyclePlant(plant.m_handle);
          }
        }
        ImGui::TreePop();
      }
  ...
  if (ImGui::TreeNode("Generate Plants...")) {
    ...
    ImGui::DragInt("Parameter Index", &parameterHandle, 1, 0,
                   static_cast<int>(target.m_distribution.m_spatialPlantParameters.size()) - 1);
    parameterHandle =
        glm::clamp(parameterHandle, 0, static_cast<int>(target.m_distribution.m_spatialPlantParameters.size()) - 1);
    if (ImGui::Button("Generate")) {
      for (int i = 0; i < plantSize; i++) {
        target.m_distribution.AddPlant(
            parameterHandle, initialRadius,
            glm::linearRand(glm::vec2(-target.m_distribution.m_spatialPlantGlobalParameters.m_maxRadius),
                            glm::vec2(target.m_distribution.m_spatialPlantGlobalParameters.m_maxRadius)));
      }
    }
    ImGui::TreePop();
  }

  ImGui::Checkbox("Simulate", &target.m_simulate);
  ...
  if (ImGui::Button("Create Trees")) {
    ...
    for (const auto& plant : target.m_distribution.m_plants) {
      if (glm::abs(plant.m_position.x - offset.x) < range && glm::abs(plant.m_position.y - offset.y) < range &&
          target.m_treeDescriptors.size() > plant.m_parameterHandle &&
          target.m_treeDescriptors.at(plant.m_parameterHandle).Get<TreeDescriptor>()) {
        ...
        tree->shoot_model.tree_growth_settings = target.m_treeGrowthSettings;
        tree->tree_descriptor_ref = target.m_treeDescriptors.at(plant.m_parameterHandle);
        if (setParent)
