...
void BakeColor(const TextureBaker::Parameters& parameters, const CompressedMapUv& compressed_map_uv,
  ...
  auto time = ApplicationContext::Get().GetTimes().Now();
  processing_time += ApplicationContext::Get().GetTimes().Now() - time;
  time = ApplicationContext::Get().GetTimes().Now();
  ...
  loading_time += ApplicationContext::Get().GetTimes().Now() - time;
  time = ApplicationContext::Get().GetTimes().Now();
