...
// (maize tassel, Scots pine, etc.).
//
// Bit-exact-parity contract: function bodies, RNG distribution construction,
// and integer constants below MUST remain byte-identical to the originals.
...
inline float EvaluatePlottedDeterministic(const evo_engine::PlottedDistribution<float>& pd, const float t,
...
inline float SampleGaussianClamped(const float mean, const float sigma, const float lo, const float hi,
                                   std::mt19937& rng) {
  if (!(sigma > 0.0f))
...
inline int SamplePoissonNonneg(const float lambda, std::mt19937& rng) {
  if (!(lambda > 0.0f))
    return std::max(0, static_cast<int>(std::round(lambda)));
...
inline int SampleCountPoissonOrFixed(const int default_count, const float poisson_lambda, const int min_count,
                                     const int max_count, std::mt19937& rng) {
  const int raw = (poisson_lambda > 0.0f) ? SamplePoissonNonneg(poisson_lambda, rng) : default_count;
