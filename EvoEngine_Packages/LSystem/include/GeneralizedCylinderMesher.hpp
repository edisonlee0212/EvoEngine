...
// emitted mesh has the same topology (segments * (stations - 1) quads) as the
...
inline float ResolveRadius(const GeneralizedCylinderMesherConfig& cfg, float arc_length, float total_length) {
  if (cfg.radius_table.empty() || total_length <= 0.0f)
    return cfg.base_radius;
  if (cfg.radius_table.size() == 1)
    return cfg.radius_table.front();
  const float t = std::clamp(arc_length / total_length, 0.0f, 1.0f);
  const float pos = t * static_cast<float>(cfg.radius_table.size() - 1);
  const int i = std::min(static_cast<int>(cfg.radius_table.size()) - 2, static_cast<int>(std::floor(pos)));
  const float u = pos - static_cast<float>(i);
  return glm::mix(cfg.radius_table[i], cfg.radius_table[i + 1], u);
}

inline float ResolveSecondaryRadius(const GeneralizedCylinderMesherConfig& cfg, float arc_length, float total_length) {
  if (cfg.secondary_radius_table.empty() || total_length <= 0.0f) {
    return cfg.secondary_base_radius;
  }
  if (cfg.secondary_radius_table.size() == 1)
    return cfg.secondary_radius_table.front();
  const float t = std::clamp(arc_length / total_length, 0.0f, 1.0f);
  const float pos = t * static_cast<float>(cfg.secondary_radius_table.size() - 1);
  const int i = std::min(static_cast<int>(cfg.secondary_radius_table.size()) - 2, static_cast<int>(std::floor(pos)));
  const float u = pos - static_cast<float>(i);
  return glm::mix(cfg.secondary_radius_table[i], cfg.secondary_radius_table[i + 1], u);
}

inline glm::vec4 ResolveStationColor(const GeneralizedCylinderMesherConfig& cfg, int station_index, int station_count) {
  if (cfg.station_color_table.empty())
    return cfg.vertex_color;
  if (cfg.station_color_table.size() == 1)
    return cfg.station_color_table.front();
  ...
  const int i = std::min(static_cast<int>(cfg.station_color_table.size()) - 2, static_cast<int>(std::floor(pos)));
  const float u = pos - static_cast<float>(i);
...
inline std::size_t SweepGeneralizedCylinder(const OrganCenterline& centerline, const CrossSectionProfile& profile,
                                            const GeneralizedCylinderMesherConfig& cfg,
                                            std::vector<VertexT>& out_vertices, std::vector<glm::uvec3>& out_indices) {
  ...
  for (int i = 0; i < n_stations; ++i) {
    ...
    const float secondary_radius = mesher_detail::ResolveSecondaryRadius(cfg, s, total_length);
    const glm::vec4 station_color = mesher_detail::ResolveStationColor(cfg, i, n_stations);
    ...
    for (const auto& ps : profile_samples) {
      const glm::vec3 local_pos =
          frame.position + ps.radial_offset.x * frame.normal + ps.radial_offset.y * frame.binormal;
      const glm::vec3 local_nrm = ps.outward_normal.x * frame.normal + ps.outward_normal.y * frame.binormal;
      VertexT v{};
  ...
  for (int station = 0; station < n_stations - 1; ++station) {
    ...
    for (int p = 0; p < last_p; ++p) {
      ...
      out_indices.emplace_back(static_cast<unsigned int>(p0), static_cast<unsigned int>(p2),
                               static_cast<unsigned int>(p1));
      out_indices.emplace_back(static_cast<unsigned int>(p1), static_cast<unsigned int>(p2),
  ...
  if (closed && n_perim >= 3) {
    ...
    for (int p = 1; p + 1 < n_perim; ++p) {
      out_indices.emplace_back(static_cast<unsigned int>(row_base), static_cast<unsigned int>(row_base + p + 1),
                               static_cast<unsigned int>(row_base + p));
    }
    // Tip cap: fan around last station.
    const std::size_t row_tip = vertex_offset + static_cast<std::size_t>(n_stations - 1) * emitted_per_station;
    for (int p = 1; p + 1 < n_perim; ++p) {
      out_indices.emplace_back(static_cast<unsigned int>(row_tip), static_cast<unsigned int>(row_tip + p),
