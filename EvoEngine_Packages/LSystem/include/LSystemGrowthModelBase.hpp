...
// Generic Curiously Recurring Template Pattern (CRTP) base for L-system growth models. Owns the algorithm skeleton
...
// invocations, profile timestamps).
...
inline double ProfileNowSeconds() {
...
class LSystemGrowthModelBase {
 public:
  ...
  [[nodiscard]] bool IsTopologyComplete() const {
    return topology_complete_;
  }
 ...
 private:
  ...
  void RefreshNodeTemporalState(TNode& node) {
    ...
    temporal.age_absolute_years = std::max(0.0f, chronological_years_ - temporal.birth_absolute_years);
  }
...
void LSystemGrowthModelBase<Derived, TGraph, TEngine>::GrowStep() {
  ...
  const double step_start = ProfileNowSeconds();
  last_growth_step_profile = {};
  bool topology_phase_ran = false;
  bool topology_changed = false;

  // Age and interpolate existing modules first.
  double phase_start = ProfileNowSeconds();
  engine_.ApplyGrowthRules(graph, rng_);
  last_growth_step_profile.apply_growth_rules_seconds = ProfileNowSeconds() - phase_start;

  // Once topology is complete, growth rules can no longer create topology symbols.
  if (!topology_complete_) {
    topology_phase_ran = true;
    phase_start = ProfileNowSeconds();
    topology_changed = engine_.ApplyTopologyRules(graph, rng_);
    last_growth_step_profile.apply_topology_rules_seconds = ProfileNowSeconds() - phase_start;

    phase_start = ProfileNowSeconds();
    if (topology_changed) {
      graph.SortLists();
    }
    last_growth_step_profile.sort_lists_seconds += ProfileNowSeconds() - phase_start;
  }
  ...
  last_growth_step_profile.update_node_info_seconds = ProfileNowSeconds() - phase_start;

  phase_start = ProfileNowSeconds();
  PropagateGeometry();
  last_growth_step_profile.propagate_geometry_seconds = ProfileNowSeconds() - phase_start;
  ...
  if (topology_phase_ran) {
    ...
    last_growth_step_profile.topology_scan_seconds = ProfileNowSeconds() - phase_start;
  }

  last_growth_step_profile.total_seconds = ProfileNowSeconds() - step_start;
}
...
template <typename Derived, typename TGraph, typename TEngine>
void LSystemGrowthModelBase<Derived, TGraph, TEngine>::GrowToGDD(const float target_gdd,
                                                                 const uint32_t max_growth_steps) {
  last_growth_steps = 0;
  last_grow_to_gdd_profile = {};
  while (accumulated_gdd < target_gdd && (max_growth_steps == 0 || last_growth_steps < max_growth_steps)) {
    GrowStep();
    last_growth_steps++;

    last_grow_to_gdd_profile.total_seconds += last_growth_step_profile.total_seconds;
    last_grow_to_gdd_profile.apply_growth_rules_seconds += last_growth_step_profile.apply_growth_rules_seconds;
    last_grow_to_gdd_profile.apply_topology_rules_seconds += last_growth_step_profile.apply_topology_rules_seconds;
    last_grow_to_gdd_profile.sort_lists_seconds += last_growth_step_profile.sort_lists_seconds;
    last_grow_to_gdd_profile.update_node_info_seconds += last_growth_step_profile.update_node_info_seconds;
    last_grow_to_gdd_profile.propagate_geometry_seconds += last_growth_step_profile.propagate_geometry_seconds;
    last_grow_to_gdd_profile.topology_scan_seconds += last_growth_step_profile.topology_scan_seconds;
  }
