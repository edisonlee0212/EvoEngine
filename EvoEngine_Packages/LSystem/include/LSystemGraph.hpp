...
template <typename GraphData, typename FlowData, typename ModuleData>
void LSystemGraph<GraphData, FlowData, ModuleData>::ReparentNode(const LNodeHandle target_handle,
                                                                 const LNodeHandle new_parent_handle) {
  assert(target_handle >= 0 && new_parent_handle >= 0 && target_handle < static_cast<int>(nodes_.size()) &&
...
void LSystemGraph<GraphData, FlowData, ModuleData>::SetParentNode(LNodeHandle target_handle,
  ...
  assert(target_handle >= 0 && parent_handle >= 0 && target_handle < static_cast<int>(nodes_.size()) &&
         parent_handle < static_cast<int>(nodes_.size()));
  auto& target_node = nodes_[target_handle];
...
void LSystemGraph<GraphData, FlowData, ModuleData>::DetachChildNode(LNodeHandle target_handle,
  ...
  assert(target_handle >= 0 && child_handle >= 0 && target_handle < static_cast<int>(nodes_.size()) &&
         child_handle < static_cast<int>(nodes_.size()));
  auto& target_node = nodes_[target_handle];
...
void LSystemGraph<GraphData, FlowData, ModuleData>::SetParentFlow(LFlowHandle target_handle,
  ...
  assert(target_handle >= 0 && parent_handle >= 0 && target_handle < static_cast<int>(flows_.size()) &&
         parent_handle < static_cast<int>(flows_.size()));
...
void LSystemGraph<GraphData, FlowData, ModuleData>::DetachChildFlow(LFlowHandle target_handle,
  ...
  assert(target_handle >= 0 && child_handle >= 0 && target_handle < static_cast<int>(flows_.size()) &&
         child_handle < static_cast<int>(flows_.size()));
  auto& target_flow = flows_[target_handle];
