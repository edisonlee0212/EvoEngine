...
inline float ComputeIntraYearCapacityWeight(float normalized_index, float base_ratio, float sigmoid_steepness,
                                            float sigmoid_midpoint, float late_decay_start,
...
inline PineNeedleCluster MakeNeedleCluster(const SampledPineParams& p, float s_along_norm, float roll_offset_deg,
  ...
  const float sampled_needle_cross_section_thickness_radius_m = sampled_needle_cross_section_thickness_max_m * 0.5f;
  ...
  const float intra_year_capacity_weight = ComputeIntraYearCapacityWeight(
      normalized_phytomer_progress, p.needle_intra_year_base_ratio, p.needle_intra_year_sigmoid_steepness,
      p.needle_intra_year_sigmoid_midpoint_fraction, p.needle_intra_year_late_decay_start_fraction,
      p.needle_intra_year_late_decay_end_scale);

  const bool is_fascicular_year = initiation_year_index >= std::max(0, p.needle_fascicular_start_year);
  const float year_length_multiplier = is_fascicular_year ? std::max(0.0f, p.needle_year2plus_length_multiplier) : 1.0f;
  const float year_width_multiplier = is_fascicular_year ? std::max(0.0f, p.needle_year2plus_width_multiplier) : 1.0f;
  const float year_thickness_multiplier =
      is_fascicular_year ? std::max(0.0f, p.needle_year2plus_thickness_multiplier) : 1.0f;
  ...
  const float inter_year_width_capacity_weight = year_width_multiplier * bud_storage_vigor_weight;
  const float inter_year_thickness_capacity_weight = year_thickness_multiplier * bud_storage_vigor_weight;
  ...
  const float adjusted_width_radius_m =
      std::max(0.0f, sampled_needle_cross_section_width_radius_m * intra_year_capacity_weight *
                         inter_year_width_capacity_weight);
  const float adjusted_thickness_radius_m =
      std::max(0.0f, sampled_needle_cross_section_thickness_radius_m * intra_year_capacity_weight *
                         inter_year_thickness_capacity_weight);
  ...
  c.branching_angle_deg = std::clamp(
      DeterministicDistributionSample(p.distributions.needle_branching_angle_deg, c.node_random, 0u, 0x2d3e4f50u), 0.0f,
      89.5f);
  const float sampled_branching_relax_gdd = std::max(
      0.0f,
      DeterministicDistributionSample(p.distributions.needle_branching_relax_gdd, c.node_random, 0u, 0x17a3c8d2u));
  ...
  for (int i = 0; i < needle_count; ++i) {
    ...
    const float profile_branching_relax_gdd =
        std::max(0.0f, DeterministicDistributionSample(p.distributions.needle_branching_relax_gdd, c.node_random,
    ...
    profile.sinusoidal_amplitude_deg = std::clamp(profile.sinusoidal_amplitude_deg * wave_amplitude_scale, 0.0f, 45.0f);
    profile.sinusoidal_frequency_cycles =
        std::clamp(profile.sinusoidal_frequency_cycles * wave_frequency_scale, 0.0f, 12.0f);
...
inline float SamplePlastochronYears(const SampledPineParams& p, std::mt19937& node_rng) {
  return std::clamp(SampleDistribution(p.distributions.plastochron_gdd, node_rng) / kPineGddPerYear, 0.05f, 4.0f);
}
    ...
    rule.produce = [params, enable_needle_topology](RuleContext<PineGraph>& ctx) -> ProductionResult<PineModuleData> {
      ...
      if (enable_needle_topology && apex.phytomers_this_year >= apex.bare_phytomers_this_year) {
        ...
        s.data.Set<PineNeedleCluster>(
            pine_detail::MakeNeedleCluster(params,
                                           /*s_along_norm=*/1.0f,
      ...
      const int branches_per_whorl = std::clamp(
          static_cast<int>(std::round(SampleDistribution(params.distributions.branches_per_whorl, node_rng))), 0, 32);
  ...
  // R4: whorl bud activation -> N lateral apices.
  {
    ...
    rule.produce = [params](RuleContext<PineGraph>& ctx) -> ProductionResult<PineModuleData> {
      ...
      for (int i = 0; i < n; ++i) {
        ...
        s.symbol_id = PineSymbol::Apex;
        const float phase =
            pine_detail::NormalizeDegrees(bud.phyllotaxis_phase + static_cast<float>(i) * roll_phyllotaxis_deg);
