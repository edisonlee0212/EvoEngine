...
namespace l_system_package::descriptor_defaults {
...
std::filesystem::path ResolveWritableDefaultsPath(const TResourceCandidates& resource_candidates,
  ...
  return fallback_relative_path.empty() ? std::filesystem::path{} : std::filesystem::absolute(fallback_relative_path);
}

bool LoadDefaultsYamlMap(const std::filesystem::path& file_path, YAML::Node& out_defaults,
