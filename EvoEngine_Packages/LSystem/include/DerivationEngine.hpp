...
class DerivationEngine {
 ...
 private:
   ...
   */
  const RuleType* FindMatchingRule(const std::vector<RuleType>& rules, const ContextType& ctx) const;

  /**
   * @brief Find the highest-priority matching rule from a prefiltered candidate list.
   * @return Pointer to the matching rule, or nullptr if none matches.
   */
  const RuleType* FindMatchingRule(const std::vector<const RuleType*>& rules, const ContextType& ctx) const;
...
const RuleType* PickFromBucket(const std::vector<const RuleType*>& bucket, std::mt19937& rng) {
  if (bucket.empty())
    return nullptr;
  if (bucket.size() == 1)
    return bucket.front();
  ...
  for (const auto* r : bucket) {
    const float w = (r && std::isfinite(r->probability) && r->probability > 0.0f) ? r->probability : 0.0f;
    if (r && r->probability != 1.0f)
      any_stochastic = true;
    total += w;
  }
...
DerivationEngine<GraphData, FlowData, ModuleData>::FindMatchingRule(const std::vector<RuleType>& rules,
...
template <typename GraphData, typename FlowData, typename ModuleData>
const typename DerivationEngine<GraphData, FlowData, ModuleData>::RuleType*
DerivationEngine<GraphData, FlowData, ModuleData>::FindMatchingRule(const std::vector<const RuleType*>& rules,
                                                                    const ContextType& ctx) const {
  // Stage 1: find max priority among matching rules.
  int max_prio = std::numeric_limits<int>::min();
  bool any = false;
  ...
  bucket.reserve(rules.size());
  for (const auto* rule : rules) {
    if (!rule || rule->priority != max_prio)
      continue;
    if (rule->predecessor_symbol >= 0 && rule->predecessor_symbol != ctx.self.symbol_id)
      continue;
    if (rule->condition && !rule->condition(ctx))
...
template <typename GraphData, typename FlowData, typename ModuleData>
bool DerivationEngine<GraphData, FlowData, ModuleData>::ApplyTopologyRules(GraphType& graph, std::mt19937& rng) const {
...
template <typename GraphData, typename FlowData, typename ModuleData>
void DerivationEngine<GraphData, FlowData, ModuleData>::ApplyGrowthRules(GraphType& graph, std::mt19937& rng) const {
