...
struct SampledPineParams {
  ...
  float bare_zone_fraction =
      0.08f;  ///< Temporal fraction at the start of each year that emits internode-only phytomers [0, 0.95).
  ...
  float needle_cross_section_thickness_max_m =
      0.0011f;                                    ///< Minor-axis full thickness (diameter) at the broadest position.
  ...
  float needle_specularity_plasticity_year1 =
      0.25f;  ///< How much micro-variation tracks maturity in year-1 cohorts [0,1].
  float needle_specularity_plasticity_year2plus =
      0.65f;  ///< How much micro-variation tracks maturity in year-2+ cohorts [0,1].
...
class ScotsPineDescriptor : public evo_engine::IAsset, public ILSystemExplorableDescriptor {
 public:
  ...
  evo_engine::SingleDistribution<float> max_branching_order{0.0f, 0.0f};
  evo_engine::SingleDistribution<float> plastochron_gdd{13.0f, 3.0f};
  evo_engine::SingleDistribution<float> max_phytomers_per_seasonal_growth{40.0f, 10.0f};

  // ===== 3. Whorl architecture =====
  evo_engine::SingleDistribution<float> branches_per_whorl{6.0f, 0.0f};
  ...
  evo_engine::SingleDistribution<float> branch_insertion_angle_deg{70.0f, 0.0f};
  ...
  evo_engine::SingleDistribution<float> internode_length_m{0.00066f, 0.0005f};
  evo_engine::SingleDistribution<float> leader_internode_thickness_m{
      0.0003f, 2.5e-05f};  ///< Main stem width (diameter), metres.
  evo_engine::SingleDistribution<float> lateral_length_ratio{0.7f, 0.0f};
  evo_engine::SingleDistribution<float> lateral_thickness_ratio{0.6f, 0.0f};
  ...
  // ===== 7. Needle layout =====
  evo_engine::SingleDistribution<float> bare_zone_fraction{0.2f, 0.05f};
  evo_engine::SingleDistribution<float> needle_count_per_cluster{2.0f, 0.0f};
  int needle_segment_count = 20;  ///< Longitudinal segments per needle (mesh stations = segments + 1).
  ...
  evo_engine::SingleDistribution<float> needle_branching_relax_gdd{2000.0f, 500.0f};
  /// [deprecated] Legacy cap-ratio field kept for schema compatibility.
  /// Needle width/thickness are now uncapped and this value is ignored.
  float needle_radius_to_stem_thickness_max_ratio = 0.5f;
  ...
  evo_engine::SingleDistribution<float> needle_lifespan_years{4.0f, 0.6666667f};
  evo_engine::SingleDistribution<float> needle_browning_years{2.0f,
                                                              0.0f};  ///< Years from senescence onset to abscission.
  evo_engine::SingleDistribution<float> needle_flush_delay_gdd{100.0f, 3.0f};
  evo_engine::SingleDistribution<float> needle_maturation_gdd{3333.0f, 30.0f};

  // ===== 9. Needle dimensions & cross-section =====
  evo_engine::SingleDistribution<float> needle_length_m{0.01f, 0.001f};
  evo_engine::PlottedDistribution<float> needle_length_maturity_curve;
  evo_engine::SingleDistribution<float> needle_cross_section_width_max_m{0.0f, 0.0f};
  evo_engine::SingleDistribution<float> needle_cross_section_thickness_max_m{0.001f, 0.0f};
  evo_engine::PlottedDistribution<float> needle_cross_section_width_profile;
  evo_engine::PlottedDistribution<float> needle_cross_section_thickness_profile;
  evo_engine::PlottedDistribution<float> needle_cross_section_temporal_maturity_curve;
  ...
  float needle_order_length_attenuation = 0.0f;
  float needle_order_radius_attenuation = 0.0f;
  float needle_order_min_length_scale = 0.1f;
  float needle_order_min_radius_scale = 0.1f;
  ...
  float needle_year2plus_length_multiplier = 4.0f;
  ...
  float needle_basal_taper_ratio_year1 = 1.0f;
  float needle_basal_taper_ratio_year2plus = 1.0f;
  ...
  evo_engine::SingleDistribution<float> needle_curvature_adaxial_bias{0.0f, 0.037f};
  evo_engine::SingleDistribution<float> needle_curvature_abaxial_bias{0.0f, 0.024f};
  evo_engine::SingleDistribution<float> needle_curvature_gradient_per_arclen{0.068f, 0.0f};
  evo_engine::SingleDistribution<float> needle_diameter_for_curvature_m{0.0033f, 0.0f};
  evo_engine::SingleDistribution<float> needle_sinusoidal_amplitude_deg{19.4f, 0.0f};
  evo_engine::SingleDistribution<float> needle_sinusoidal_frequency_cycles{2.0f, 0.0f};
  evo_engine::SingleDistribution<float> needle_sinusoidal_phase_randomness_deg{5.0f, 0.0f};
  ...
  evo_engine::SingleDistribution<float> needle_density_kg_m3{110.0f, 0.0f};
  evo_engine::SingleDistribution<float> gravity_m_s2{9.3f, 0.0f};
  ...
  // inspector.
  glm::vec4 main_stem_color_rgba{0.7861458f, 0.9165798f, 0.47310424f, 1.0f};       ///< Young main-stem bark tint.
  glm::vec4 main_stem_old_color_rgba{0.7009804f, 0.45447198f, 0.18555366f, 1.0f};  ///< Aged main-stem bark tint.
  glm::vec4 needle_color_rgba{0.0f, 0.7455683f, 0.051418442f, 1.0f};               ///< Young needle tint.
  glm::vec4 needle_old_color_rgba{0.77059436f, 0.5399785f, 0.0f, 1.0f};            ///< Aged/senescent needle tint.
