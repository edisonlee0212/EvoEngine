...
class ParamSpaceExplorer {
 public:
  ...
  void AddAxis(const std::string& label, const std::string& short_label, float min_val, float max_val,
               std::function<float()> getter, std::function<void(float)> setter);

  void AddSingle(const std::string& label_prefix, const std::string& short_prefix,
                 evo_engine::SingleDistribution<float>& dist, float mean_min, float mean_max, float deviation_max);

  void AddPlotted(const std::string& label_prefix, const std::string& short_prefix,
                  evo_engine::PlottedDistribution<float>& dist);

  void AddCurve(const std::string& label_prefix, const std::string& short_prefix, evo_engine::Curve2D& curve);
