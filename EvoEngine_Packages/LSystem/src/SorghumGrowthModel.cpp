...
// -----------------------------------------------------------------------------
void UpdateLeafMaturityState(SorghumLeaf& leaf, const float thermal_now_years, const float absolute_age_years) {
...
// -----------------------------------------------------------------------------
bool UpdateLeafChronologicalAging(SorghumLeaf& leaf, const float absolute_age_years) {
  ...
  const float lifespan_jitter = (leaf.node_random - 0.5f) * 2.0f * kNodeRandomJitterYears;
  ...
  const float senescence_age_years =
      leaf.maturity_reached
          ? std::max(0.0f, clamped_age_years - std::max(0.0f, leaf.chronological_age_at_maturity_years))
          : 0.0f;
  ...
  if (leaf.maturity_reached && senescence_age_years > effective_lifespan_years) {
    ...
    const float effective_wilting_years = std::max(0.05f, wilting_years * wilting_scale);
    const float phase =
        (wilting_years > 0.0f)
            ? std::clamp((senescence_age_years - effective_lifespan_years) / effective_wilting_years, 0.0f, 1.0f)
            : 1.0f;
  ...
  return leaf.age_years != previous_age_years || std::abs(leaf.senescence_phase - previous_senescence) > 1.0e-5f ||
...
void SorghumGrowthModel::Initialize(const SorghumLSDescriptor& descriptor, const unsigned int seed,
  ...
  // Seed one persistent root and one main apex. Primary tiller buds are
  // created later by the main-culm rule at their actual leaf-axil nodes.
  graph = SorghumGraph(1);
  graph.data.main_internode_target_lengths.reserve(static_cast<size_t>(std::max(1, sampled.total_phytomer_count)));
  for (int rank = 0; rank < std::max(1, sampled.total_phytomer_count); ++rank) {
    const float position = ComputeSorghumAxisRankPosition(rank, std::max(1, sampled.total_phytomer_count));
    graph.data.main_internode_target_lengths.emplace_back(
        std::max(0.001f, SamplePlotted(sampled.internode_length, position, rng_)));
  }
  const int basal_count =
      std::min(sampled.total_phytomer_count,
               sampled.tiller_origin_ranks.empty()
                   ? 0
                   : *std::max_element(sampled.tiller_origin_ranks.begin(), sampled.tiller_origin_ranks.end()));
  const float basal_path = std::accumulate(graph.data.main_internode_target_lengths.begin(),
                                           graph.data.main_internode_target_lengths.begin() + basal_count, 0.0f);
  const float upper_path = std::accumulate(graph.data.main_internode_target_lengths.begin() + basal_count,
                                           graph.data.main_internode_target_lengths.end(), 0.0f);
  if (basal_path > 0.0f && upper_path > 0.0f && basal_path > upper_path / 3.0f) {
    const float basal_scale = upper_path / (3.0f * basal_path);
    for (int rank = 0; rank < basal_count; ++rank) {
      graph.data.main_internode_target_lengths[static_cast<size_t>(rank)] *= basal_scale;
    }
  }
  ...
  apex.axis_id = 0;
  apex.reference_main_phytomer_count = sampled.total_phytomer_count;
  apex.axis_phytomer_count = sampled.total_phytomer_count;
  ...
  // Topology budget upper bound: main phytomers plus every selected primary
  // tiller's sampled peer-axis leaf budget.
  const int main_phytomers = std::max(1, sampled.total_phytomer_count);
  const int tiller_phytomers = static_cast<int>(sampled.tiller_origin_ranks.size()) *
                               std::max(1, static_cast<int>(std::ceil(main_phytomers * 1.1f)));
  max_topology_steps_ = std::max(64, (main_phytomers + tiller_phytomers) * 4 + 64);

  initialized_ = true;
  topology_complete_ = false;
}

void SorghumGrowthModel::FinalizeSnapshotMorphology() {
  if (!initialized_)
    return;
  graph.data.main_expanded_leaf_count = std::max(graph.data.main_expanded_leaf_count, sampled.total_phytomer_count);

  for (int step = 0; step < max_topology_steps_; ++step) {
    bool pending = false;
    graph.SortLists();
    for (const auto handle : graph.PeekSortedNodeList()) {
      auto& node = graph.RefNode(handle);
      if (node.data.Is<SorghumTillerBud>()) {
        pending = true;
      } else if (node.data.Is<SorghumApex>()) {
        auto& apex = node.data.Get<SorghumApex>();
        if (apex.vigor > 0) {
  ...
  for (const auto handle : graph.PeekSortedNodeList()) {
    ...
    if (node.data.Is<SorghumInternode>()) {
      ...
      internode.thickness = internode.target_thickness;
      ...
      leaf.blade_length = leaf.target_blade_length;
      leaf.blade_max_width = leaf.target_blade_max_width;
      leaf.blade_thickness = leaf.target_blade_thickness;
      leaf.sheath_thickness = leaf.target_sheath_thickness;
      leaf.sheath_length = leaf.target_sheath_length;
      leaf.neck_length = leaf.target_neck_length;
      leaf.sheath_end_width_ratio = leaf.target_sheath_end_width_ratio;
      leaf.neck_end_width_ratio = leaf.target_neck_end_width_ratio;
...
void SorghumGrowthModel::UpdateNodeInfoImpl(LGraphNode<SorghumModuleData>& node) {
  ...
  if (node.data.Is<SorghumLeaf>()) {
    ...
    if (leaf.order == 0 && leaf.growth_progress >= 0.95f) {
      graph.data.main_expanded_leaf_count = std::max(graph.data.main_expanded_leaf_count, leaf.rank + 1);
    }
...
bool SorghumGrowthModel::IsDevelopmentalSymbolImpl(const LGraphNode<SorghumModuleData>& node) const {
...
bool SorghumGrowthModel::UpdateNodeAgingOnlyImpl(LGraphNode<SorghumModuleData>& node) {
  if (node.data.Is<SorghumLeaf>()) {
    auto& leaf = node.data.Get<SorghumLeaf>();
    const float thermal_now_years = accumulated_gdd / kSorghumGddPerYear;
    UpdateLeafMaturityState(leaf, thermal_now_years, node.info.temporal.age_absolute_years);
    return UpdateLeafChronologicalAging(leaf, node.info.temporal.age_absolute_years);
  }
  return false;
}
