...
bool descriptor_defaults::LoadDefaultsYamlMap(const std::filesystem::path& file_path, YAML::Node& out_defaults,
  ...
  try {
    ...
    EVOENGINE_WARNING("Failed to load " + descriptor_name_for_logging + " defaults from " + file_path.string() + ": " +
                      std::string(e.what()));
    return false;
  }
}