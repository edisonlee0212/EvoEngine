...
void ParamSpaceExplorer::PushTrailPoint() {
  ...
  if (trail_count_ < kTrailCapacity)
    trail_count_++;
}
...
void ParamSpaceExplorer::AddAxis(const std::string& label, const std::string& short_label, float min_val, float max_val,
...
void ParamSpaceExplorer::AddSingle(const std::string& label_prefix, const std::string& short_prefix,
                                   evo_engine::SingleDistribution<float>& dist, float mean_min, float mean_max,
                                   float deviation_max) {
  auto* d = &dist;
  AddAxis(
      label_prefix + ".mean", short_prefix + "M", mean_min, mean_max,
      [d]() {
        return d->mean;
      },
      [d](float v) {
  ...
  AddAxis(
      label_prefix + ".deviation", short_prefix + "D", 0.0f, dev_hi,
      [d]() {
        return d->deviation;
      },
      [d](float v) {
        d->deviation = std::max(0.0f, v);
      });
}

void ParamSpaceExplorer::AddCurve(const std::string& label_prefix, const std::string& short_prefix,
  ...
  for (size_t i = 0; i < values.size(); i++) {
    ...
    AddAxis(
        label_prefix + idx + ".x", short_prefix + "X", xr_min, xr_max,
        [curve_ptr, i]() {
    ...
    AddAxis(
        label_prefix + idx + ".y", short_prefix + "Y", yr_min, yr_max,
        [curve_ptr, i]() {
          auto& pts = curve_ptr->UnsafeGetValues();
...
void ParamSpaceExplorer::AddPlotted(const std::string& label_prefix, const std::string& short_prefix,
  ...
  AddAxis(
      label_prefix + ".mean.min", short_prefix + "Mm", mean_min_lo, mean_min_hi,
      [pd]() {
        return pd->mean.min_value;
      },
      [pd](float v) {
        pd->mean.min_value = v;
      });
  AddAxis(
      label_prefix + ".mean.max", short_prefix + "Mx", mean_min_lo, mean_min_hi,
      [pd]() {
        return pd->mean.max_value;
      },
      [pd](float v) {
        pd->mean.max_value = v;
      });
  ...
  AddAxis(
      label_prefix + ".deviation.min", short_prefix + "Dm", dev_min_lo, dev_min_hi,
      [pd]() {
        return pd->deviation.min_value;
      },
      [pd](float v) {
        pd->deviation.min_value = v;
      });
  AddAxis(
      label_prefix + ".deviation.max", short_prefix + "Dx", dev_min_lo, dev_min_hi,
      [pd]() {
        return pd->deviation.max_value;
      },
      [pd](float v) {
        pd->deviation.max_value = v;
      });
...
void ParamSpaceExplorer::RebuildAxes() {
  ...
  if (!axes_.empty()) {
    ...
    visible_axis_offset_ =
        std::clamp(visible_axis_offset_, 0, std::max(0, static_cast<int>(axes_.size()) - visible_axis_count_));
    PushTrailPoint();
  }
}
...
bool ParamSpaceExplorer::Tick(float dt) {
  if (!bound_desc_ || !playing || mode == ParamMotionMode::Stopped || axes_.empty())
    return false;
...
void ParamSpaceExplorer::DrawParallelCoords(float width, float height, const std::vector<int>& axis_indices) {
  ...
  for (int i = 0; i < axis_count; i++) {
    ...
    dl->AddText(ImVec2(x - ts.x * 0.5f, y_bot + 2.0f), IM_COL32(140, 140, 150, 255),
                axes_[axis_idx].short_label.c_str());
  }
  ...
  for (int t = 0; t < trail_count_; t++) {
    ...
    for (int i = 0; i < axis_count - 1; i++) {
      ...
      dl->AddLine(axis_xy(i, trail_[idx][a]), axis_xy(i + 1, trail_[idx][b]), color, 1.0f);
    }
  }
...
bool ParamSpaceExplorer::OnInspect() {
  ...
  if (mode == ParamMotionMode::LinearInterp) {
    ...
    ImGui::SameLine();
    ImGui::Text("A:%s B:%s", has_snapshot_a_ ? "set" : "---", has_snapshot_b_ ? "set" : "---");
    if (!has_snapshot_a_ || !has_snapshot_b_) {
      ImGui::TextColored(ImVec4(1, 1, 0, 1), "Set both A and B snapshots before playing.");
    }
  }
  ...
  ImGui::DragInt("Visible Axis Count", &visible_axis_count_, 1.0f, 2, std::max(2, static_cast<int>(axes_.size())));
