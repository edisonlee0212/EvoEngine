...
TEST_F(UniverseStarDemo, CameraOverridesDeepCopySettingsAndRestoreOriginalReferences) {
  ...
  main_camera->camera_settings.near_distance = 0.1f;
  scene_camera->camera_settings.near_distance = 200;
  ...
  EXPECT_FLOAT_EQ(main_camera->camera_settings.near_distance, 0.1f);
  EXPECT_FLOAT_EQ(scene_camera->camera_settings.near_distance, 200);
  ...
  main_camera->camera_settings.near_distance = 0.25f;
  main_camera->camera_settings.far_distance = 777;
  main_override.Apply(main_camera);
  EXPECT_EQ(main_stack, main_camera->post_processing_stack_ref.Get<PostProcessingStack>());
  EXPECT_FALSE(main_stack->enable_tone_mapping);
  EXPECT_FLOAT_EQ(main_camera->camera_settings.near_distance, 0.25f);
  EXPECT_FLOAT_EQ(main_camera->camera_settings.far_distance, 777);
...
TEST(UniverseStarDemoDefaults, AuthoringDefaultsAndExplicitSerializedValues) {
  ...
  EXPECT_DOUBLE_EQ(0.1, cluster.radius_min);
  EXPECT_DOUBLE_EQ(15, cluster.radius_max);
  EXPECT_DOUBLE_EQ(1.0 / 6.0, cluster.radius_deviation);
  EXPECT_DOUBLE_EQ(10, cluster.star_minimum_distance);
  EXPECT_DOUBLE_EQ(10000000, cluster.disk_diameter);
