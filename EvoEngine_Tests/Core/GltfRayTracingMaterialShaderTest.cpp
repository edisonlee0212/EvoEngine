...
TEST(GltfRayTracingMaterial, CameraAnyHitAppliesGltfAlphaCutoff) {
  ...
  EXPECT_NE(any_hit.find("EE_GLTF_RASTER_OPACITY_LOD0(material_index, tex_coord, tex_coord, vertex_color.a)"),
            std::string::npos);
...
TEST(GltfRayTracingMaterial, RayShadersUseCanonicalMaterialBlockOnly) {
  ...
  EXPECT_NE(evaluator.find("textureLod(EE_TEXTURE_2DS[nonuniformEXT(texture_info.index)], uv, EE_GLTF_TEXTURE_LOD)"),
            std::string::npos);
  EXPECT_NE(evaluator.find("float tex_grad"), std::string::npos);
...
TEST(GltfRayTracingMaterial, RayTracedTextureLodUsesRayFootprintGradients) {
  ...
  EXPECT_NE(evaluator.find("fallback, float tex_grad"), std::string::npos);
  EXPECT_NE(evaluator.find("texture_info.uv_transform * vec3(tex_grad, 0.0, 0.0)"), std::string::npos);
  EXPECT_NE(evaluator.find("textureGrad(EE_TEXTURE_2DS[nonuniformEXT(texture_info.index)], uv, ddx_uv, ddy_uv)"),
            std::string::npos);
  EXPECT_NE(evaluator.find("EE_EVALUATE_GLTF_RASTER_SURFACE("
                           "\n    uint material_index, vec2 tex_coord_0, vec2 tex_coord_1, float tex_grad"),
            std::string::npos);

  EXPECT_NE(bsdf.find("vec3 geometric_normal, bool is_inside, float tex_grad"), std::string::npos);
  EXPECT_NE(bsdf.find("EE_EVALUATE_GLTF_RASTER_SURFACE(material_index, tex_coord_0, tex_coord_1, tex_grad)"),
            std::string::npos);
  ...
  for (const auto* source : {&raygen, &ray_query, &any_hit}) {
    EXPECT_EQ(source->find("#define EE_GLTF_USE_EXPLICIT_TEXTURE_LOD"), std::string::npos);
    EXPECT_EQ(source->find("#define EE_GLTF_TEXTURE_LOD 0.0"), std::string::npos);
    EXPECT_NE(source->find("float EE_CAMERA_TEXEL_DENSITY"), std::string::npos);
    ...
    EXPECT_NE(source->find("EE_CAMERA_TEXTURE_GRAD"), std::string::npos);
  }
  ...
  EXPECT_NE(raygen.find("hit.tex_grad = EE_CAMERA_TEXTURE_GRAD(ray_cone_width"), std::string::npos);
  EXPECT_NE(raygen.find("EE_EVALUATE_GLTF_RASTER_SURFACE(hit.material_index, tex_coord, tex_coord, hit.tex_grad)"),
            std::string::npos);
  EXPECT_NE(raygen.find("is_inside, hit.tex_grad"), std::string::npos);
  EXPECT_NE(ray_query.find("EE_CAMERA_RAY_QUERY_CANDIDATE_SURFACE(ray_query, direction, material_index, tex_coord, "
                           "vertex_color)"),
            std::string::npos);
  EXPECT_NE(ray_query.find("EE_GLTF_RASTER_OPACITY_LOD0(material_index, tex_coord, tex_coord, vertex_color.a)"),
            std::string::npos);
  EXPECT_NE(ray_query.find("EE_GLTF_RASTER_SHADOW_TRANSMISSION_LOD0("), std::string::npos);
  EXPECT_NE(ray_query.find("hit.tex_grad = EE_CAMERA_TEXTURE_GRAD(ray_cone_width"), std::string::npos);
  EXPECT_NE(ray_query.find("hit.surface = EE_EVALUATE_GLTF_RASTER_SURFACE(hit.material_index, tex_coord, tex_coord, "
                           "hit.tex_grad)"),
            std::string::npos);
  ...
  EXPECT_NE(any_hit.find("EE_GLTF_RASTER_OPACITY_LOD0(material_index, tex_coord, tex_coord, vertex_color.a)"),
            std::string::npos);
  EXPECT_NE(any_hit.find("EE_GLTF_RASTER_SHADOW_TRANSMISSION_LOD0("), std::string::npos);
  EXPECT_EQ(any_hit.find("EE_EVALUATE_GLTF_RASTER_SURFACE(material_index, tex_coord, tex_coord, tex_grad)"),
            std::string::npos);
...
TEST(GltfRayTracingMaterial, CameraRaygenOwnsPathTracingLoop) {
  ...
  EXPECT_NE(source.find("EE_EVALUATE_GLTF_RASTER_SURFACE"), std::string::npos);
  EXPECT_NE(source.find("const float tangent_handedness = v0.vertex_info3 < 0.0f ? -1.0f : 1.0f"), std::string::npos);
  ...
  EXPECT_NE(source.find("hit.tangent, hit.bitangent, hit.geometric_normal"), std::string::npos);
...
TEST(GltfRayTracingMaterial, RayTracingBsdfUsesReferenceLayeredLobeModel) {
  ...
  EXPECT_NE(bsdf.find("weights.specular_reflection = base_weight * clamp(dielectric_fresnel"), std::string::npos);
