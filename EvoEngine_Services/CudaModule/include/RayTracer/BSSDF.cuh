...
static __forceinline__ __device__ void SSAnyHit() {
  ...
  SSPerRayData &perRayData = *GetRayDataPointer<SSPerRayData>();
  if (perRayData.handle != sbtData.handle) {
    optixIgnoreIntersection();
  }
  if (perRayData.m_recordSize >= 4)
    optixTerminateRay();
  ...
  static_cast<SurfaceMaterial *>(sbtData.material)
...
static __forceinline__ __device__ bool BSSRDF(float metallic, Random &random, float radius, unsigned long long handle,
                                              OptixTraversableHandle traversable, const glm::vec3 &inPosition,
                                              const glm::vec3 &inDirection, const glm::vec3 &inNormal,
  ...
  glm::vec3 samplePosition = diskCenter + diskRadius * glm::rotate(glm::vec3(diskNormal.y, diskNormal.z, diskNormal.x),
  ...
  optixTrace(traversable, make_float3(samplePosition.x, samplePosition.y, samplePosition.z),
             ...
             static_cast<int>(RayType::SpacialSampling),  // SBT offset
             static_cast<int>(RayType::RayTypeCount),     // SBT stride
             static_cast<int>(RayType::SpacialSampling),  // missSBTIndex
             u0, u1);
