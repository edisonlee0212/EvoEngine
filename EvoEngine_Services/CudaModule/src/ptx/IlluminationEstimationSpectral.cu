...
static __forceinline__ __device__ void IlluminationSpectralAnyHitFunc() {
  // TODO(illumination-spectral): Add wavelength-aware alpha/transmission handling.
}

static __forceinline__ __device__ void IlluminationSpectralClosestHitFunc() {
...
static __forceinline__ __device__ void IlluminationSpectralMissFunc() {
  auto &per_ray_data = *GetRayDataPointer<PerRayData<glm::vec3>>();
  per_ray_data.energy = glm::vec3(0.0f);
...
extern "C" __global__ void __closesthit__IES_R() {
  IlluminationSpectralClosestHitFunc();
}
...
extern "C" __global__ void __anyhit__IES_R() {
  IlluminationSpectralAnyHitFunc();
}
