...
void StarClusterSystem::PushStars(StarClusterPattern &pattern, const size_t &amount) {
  auto scene = GetScene();
  counter_ = 0;
  auto stars = scene->CreateEntities(star_archetype_, amount, "Star");
...
void StarClusterSystem::Start() {
  ...
  if (renderer_front_.Get().GetIndex() == 0 && renderer_back_.Get().GetIndex() == 0) {
    ...
    auto imr = scene->GetOrSetPrivateComponent<Particles>(renderer_front_.Get()).lock();
    ...
    ltw.SetScale(glm::vec3(1.0f));
    auto imr2 = scene->GetOrSetPrivateComponent<Particles>(renderer_back_.Get()).lock();
